...
function hasPermission(user, required) {
  ...
  if (required === "/material-movement" && ["/material-movement/return", "/material-movement/reel-return", "/production/pending-material-return"].some((path2) => list.includes(path2))) return true;
...
async function fetchFirmWiseNpdItems(db, options) {
  ...
  try {
    ...
    const [rows2] = await db.query(`SELECT n.* FROM \`npd\` n ${whereSql} ORDER BY n.npdId ASC LIMIT ? OFFSET ?`, [...params, options.limit, options.offset]);
    const [countRows] = await db.query(`SELECT COUNT(*) AS total FROM \`npd\` n ${whereSql}`, params);
    base = { rows: rows2, total: Number(countRows[0]?.total || 0) };
  }
  ...
  const stocks = /* @__PURE__ */ new Map();
  const ensure = (itemId, firmId) => {
    if (!firmId) return null;
    if (!stocks.has(itemId)) stocks.set(itemId, /* @__PURE__ */ new Map());
...
const INTER_FIRM_COLUMNS = [
  ["orderFirmId", "VARCHAR(36)"],
  ["sourceFirmId", "VARCHAR(36)"],
  ["destinationFirmId", "VARCHAR(36)"],
  ["sourceTransactionType", "VARCHAR(80)"],
  ["sourceTransactionId", "VARCHAR(36)"],
  ["interFirmFlow", "VARCHAR(10)"],
  ["autoGenerated", "VARCHAR(10)"],
  ["mrrSource", "VARCHAR(30)"],
  ["linkedGateEntryId", "VARCHAR(36)"],
  ["linkedMrrId", "VARCHAR(36)"],
  ["linkedInvoiceId", "VARCHAR(36)"]
];
      ...
      try {
        ...
        if (tableName === "material_in") {
          ...
          if (!String(data.transactionNo || "").trim()) {
            data.transactionNo = await generateLockedMrrNo(db, String(data.date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)));
          }
        }
        ...
        if (tableName === "materials" && materialOpening) {
          ...
          try {
            ...
            await conn.query(
              ...
              [
                String(data.id || ""),
                materialOpening.firmId,
                materialOpening.qty,
                materialOpening.rate,
                materialOpening.value,
                data.updatedBy || null,
                data.updateTimestamp || null
              ]
            );
            await conn.commit();
          } catch (error) {
        ...
        if (tableName === "invoices" && String(data.interFirmFlow || "").trim().toLowerCase() === "yes") {
          ...
          if (pendingId) {
            await db.query("UPDATE inter_firm_pending_invoices SET status = 'Invoiced', linkedInvoiceId = ?, updateTimestamp = ? WHERE id = ?", [String(data.id || ""), (/* @__PURE__ */ new Date()).toISOString(), pendingId]);
  ...
  try {
    ...
    for (const [erpCode, rowsForMaterial] of rowsByErp.entries()) {
      ...
      for (const [firmId, firmRows2] of rowsByFirm) {
        ...
        const addedQty = firmRows2.reduce((sum, row) => sum + row.weightKg, 0);
        const addedValue = firmRows2.reduce((sum, row) => sum + row.weightKg * row.openingRate, 0);
...
app.post("/api/material-firm-openings", requireAuth, async (req, res) => {
  try {
    ...
    if (!materialRows[0]?.id) return res.status(404).json({ error: "Material was not found." });
...
app.post("/api/reel-transfers/execute", async (req, res) => {
  ...
  const transferDate = String(req.body?.date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)).trim();
  ...
  try {
    ...
    const corrugationRows = (productionId) => processing.filter(
    ...
    if (!Number.isFinite(windowHours) || windowHours <= 0 || Date.now() > sourceFull.time + windowHours * 60 * 60 * 1e3) {
    ...
    if (Number(targetIssueRows[0]?.count || 0) > 0) throw new Error("The target job already has a reel issue.");
    ...
    const balanceBySlip = /* @__PURE__ */ new Map();
    ...
    if (averageNotionalKg <= 4e-3) throw new Error("No positive notional reel balance is available for transfer.");
    const selected = packingSlipIds.map((packingSlipId) => {
      ...
      if (!issue || balance <= 4e-3 || weightKg <= 0) throw new Error("A selected reel no longer has an available source balance.");
