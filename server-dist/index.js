...
async function generateLockedMrrNo(db, dateStr) {
    ...
    if (!fy)
        throw new Error("Could not generate MRR number because date is invalid.");
    const key = `MI/${fy}`;
    await db.query(`INSERT INTO transaction_counters (counterKey, lastValue)
     ...
     ON DUPLICATE KEY UPDATE lastValue = GREATEST(lastValue, VALUES(lastValue))`, [key, `${key}/%`]);
    ...
    return `MI/${fy}/${String(Number(rows[0]?.nextValue || 0)).padStart(5, "0")}`;
}
async function generateLockedGateEntryNo(db, dateStr) {
    const fy = getShortFinancialYear(dateStr);
    if (!fy)
        throw new Error("Could not generate Gate Entry number because date is invalid.");
    const key = `GE/${fy}`;
    await db.query(`INSERT INTO transaction_counters (counterKey, lastValue)
    ...
    try {
       ...
       ORDER BY ip.updateTimestamp DESC LIMIT 2`, [sourceTransactionType, productionId, sourceFirmId, destinationFirmId]);
        ...
        if (!mrr) {
            ...
            const transactionNo = await generateLockedMrrNo(conn, String(sourceRecord.date || production.date || now.slice(0, 10)));
