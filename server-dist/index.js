...
async function automateInterFirmProduction(db, productionId, sourceType = "Production") {
    const conn = await db.getConnection();
    try {
        await conn.beginTransaction();
        ...
        if (sourceType === "Production Processing") {
            const [processingRows] = await conn.query("SELECT * FROM `production_processing` WHERE id = ? LIMIT 1 FOR UPDATE", [productionId]);
            sourceRecord = processingRows[0] || production;
        }
        const sourceFirmId = String(sourceType === "Production Processing"
        ...
        if (!gate) {
            ...
            await conn.query(`INSERT INTO gate_entries (id, firmId, date, supplierId, purpose, invoiceNo, invoiceValue, truckNo, mrrId, mrrDate, mrrNo, status, autoGenerated, interFirmFlow, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, updatedBy, updateTimestamp) VALUES (?, ?, ?, ?, 'Material Receipt', '', ?, '', ?, ?, ?, 'Active', 'Yes', 'Yes', ?, ?, ?, ?, 'System', ?)`, [gateId, destinationFirmId, String(production.date || now.slice(0, 10)), sourceFirmId, qty * rate, mrr.id, String(production.date || now.slice(0, 10)), mrr.transactionNo, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, now]);
            gate = { id: gateId };
        }
             ...
             LIMIT 1`, [normalizedUserId, String(data.id || "")]);
               ...
               ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate),
                ...
                if (tableName === "production_processing" && String(data.completionStatus || "").trim().toLowerCase() === "full") {
                    ...
                    if (String(parentRows[0]?.interFirmFlow || "").toLowerCase() === "yes") {
