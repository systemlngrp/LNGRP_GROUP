...
async function getUnitOneFirm(db) {
...
async function migrateReelInventoryToUnitOne(db) {
    ...
    `, [unitOne.id]);
      ...
      SELECT m.id AS materialId,
        ...
        COALESCE(SUM(ps.weightKg), 0) AS qty,
        COALESCE(SUM(ps.weightKg * COALESCE(ps.openingRate, 0)), 0) AS value
      FROM materials m
      LEFT JOIN material_in_packing_slips ps
        ON ps.materialId = m.id AND TRIM(COALESCE(ps.materialInId, '')) = 'OPENING'
      WHERE m.type = 'Reel'
      GROUP BY m.id
    `);
        for (const row of reelRows) {
            const materialId = String(row.materialId || "");
            const reelCount = Number(row.reelCount || 0);
            if (!materialId || reelCount <= 0)
                continue;
            const qty = Number(row.qty || 0);
            const value = Number(row.value || 0);
            const rate = qty > 0 ? Number((value / qty).toFixed(2)) : 0;
            await conn.query("DELETE FROM material_firm_openings WHERE materialId = ?", [materialId]);
         ...
         ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate), openingValue = VALUES(openingValue), updatedBy = VALUES(updatedBy), updateTimestamp = VALUES(updateTimestamp)`, [materialId, unitOne.id, qty, rate, value, new Date().toISOString()]);
        }
        const [legacyOpeningRows] = await conn.query(`
      SELECT mfo.materialId, COALESCE(SUM(mfo.openingQty), 0) AS qty,
        COALESCE(SUM(mfo.openingValue), 0) AS value
      FROM material_firm_openings mfo
      JOIN materials m ON m.id = mfo.materialId AND m.type = 'Reel'
      WHERE NOT EXISTS (
        SELECT 1 FROM material_in_packing_slips ps
        WHERE ps.materialId = mfo.materialId AND TRIM(COALESCE(ps.materialInId, '')) = 'OPENING'
      )
      GROUP BY mfo.materialId
    `);
        for (const row of legacyOpeningRows) {
            const materialId = String(row.materialId || "");
            const qty = Number(row.qty || 0);
            const value = Number(row.value || 0);
            const rate = qty > 0 ? Number((value / qty).toFixed(2)) : 0;
            await conn.query("DELETE FROM material_firm_openings WHERE materialId = ?", [materialId]);
            await conn.query(`INSERT INTO material_firm_openings (id, materialId, firmId, openingQty, openingRate, openingValue, updatedBy, updateTimestamp)
         VALUES (UUID(), ?, ?, ?, ?, ?, 'System Migration', ?)`, [materialId, unitOne.id, qty, rate, value, new Date().toISOString()]);
        }
        await conn.commit();
    }
    catch (error) {
...
function verifyToken(token) {
    ...
    if (sigPart.length !== expectedPart.length)
        return null;
    if (!crypto.timingSafeEqual(Buffer.from(sigPart), Buffer.from(expectedPart)))
        return null;
    try {
...
function hasPermission(user, required) {
    ...
    if (user.role === "TruckDriver")
        return required === "/truck/status-update";
    const list = user.menuAccess || [];
    if (list.includes("*"))
        return true;
    if (required === "/material-movement/reel-transfer" && list.includes("/material-movement/issue"))
        return true;
    if (required === "/material-movement/reel-transfer" && list.includes("/production/pending-job-transfer"))
        return true;
    if (required === "/material-movement" &&
        ["/material-movement/return", "/material-movement/reel-return", "/production/pending-material-return"].some((path) => list.includes(path)))
        return true;
    if (required === "/production-processing" &&
    ...
    if (["/orders", "/masters/companies", "/masters/npd"].includes(required) &&
    ...
    return list.some((entry) => {
        ...
        if (entry === required)
            return true;
        if (entry !== "/" && required.startsWith(`${entry}/`))
            return true; // user granted section access like "/production"
        if (required !== "/" && entry.startsWith(`${required}/`))
            return true; // user granted a specific page under this section
        if (entry.endsWith("/*") && required.startsWith(entry.slice(0, -1)))
            return true;
        return false;
    });
}
...
app.post("/api/auth/login", async (req, res) => {
    ...
    try {
        ...
        const row = rows[0];
        if (!row) {
            const [truckRows] = await db.query("SELECT * FROM `trucks` WHERE driverLoginId = ? LIMIT 1", [identifier]);
...
app.post("/api/material-in/ai-fetch", async (req, res) => {
    ...
    if (photos.length === 0)
        return res.status(400).json({ error: "At least one invoice photo or PDF is required." });
    if (photos.length > 8)
        return res.status(400).json({ error: "Upload up to 8 invoice files at a time." });
    const parts = [{
...
Use Reel only for paper reel/roll items with reel, roll, kraft, paper, GSM, BF, deckle, or size indicators. For Reel lines, if the item text contains patterns like Size 178 / GSM 140 / BF 18 or 99 CM X GSM: 120 X BF: 18, populate numeric size, gsm, and bf exactly from that text. For Reel color, extract only the original color printed on the invoice or packing slip, and only when it is exactly one of these values: NS, Natural, LG, Duplex. Search item descriptions, color fields, PO/order text, and packing slip rows for those exact tokens. Do not infer, translate, map, or suggest colors from words like kraft, brown, white, or any unsupported code. If NS, Natural, LG, or Duplex is not explicitly visible, return color as blank. For Reel lines, extract every visible individual reel row under that item/spec into reels with supplierReelNo and weightKg; include supplierPoNo per reel when visible, otherwise blank. Keep qty as the total invoice weight for the item/spec. If individual reel rows are not visible, use reels: []. Use Other for consumables, spares, chemicals, packing material, and non-reel goods. Dates must be ISO YYYY-MM-DD when visible. Numbers must be numeric. If unsure, leave fields blank or null and lower confidence.`
    ...
    if (parts.length === 1)
        return res.status(400).json({ error: "No readable file data was supplied." });
    try {
...
app.post("/api/npd-sync", async (req, res) => {
    const providedSecret = stringOrEmpty(req.headers["x-npd-sync-secret"] ||
    ...
    console.log(`${NPD_SYNC_LOG_PREFIX} Request received`, JSON.stringify({
        tabName,
        syncMode,
        spreadsheetName,
        spreadsheetId,
    ...
    if (syncMode !== "full_finalize" && tabName === "Companies") {
        const hasIdHeader = rowsPayload.some((row) => Object.prototype.hasOwnProperty.call(row || {}, "Id") ||
            Object.prototype.hasOwnProperty.call(row || {}, "id") ||
            Object.prototype.hasOwnProperty.call(row || {}, "ID"));
        const hasCompanyHeader = rowsPayload.some((row) => Object.prototype.hasOwnProperty.call(row || {}, "Company") ||
        ...
        missingRequiredHeaders = config.requiredHeaders.filter((header) => rowsPayload.length > 0 && !rowsPayload.some((row) => Object.prototype.hasOwnProperty.call(row || {}, header)));
    }
    ...
    if (duplicateIds.length > 0) {
        console.warn(`${NPD_SYNC_LOG_PREFIX} Deduplicated repeated IDs in batch payload`, JSON.stringify({ duplicateIds, keptRows: validRows.length }));
    }
    const conn = await db.getConnection();
    try {
        await conn.beginTransaction();
        const [existingRows] = await conn.query(config.table === "npd"
        ...
        existingRows.forEach((row) => {
            ...
            if (syncId)
                existingBySyncId.set(syncId, row);
            if (config.table === "npd") {
                const erp = stringOrEmpty(row.erp);
                if (erp)
                    existingByBusinessKey.set(`erp:${erp}`, row);
            }
        });
        let inserted = 0;
        let updated = 0;
        if (syncMode !== "full_finalize") {
            for (const entry of validRows) {
                ...
                const updateColumns = columns
                    ...
                    .join(", ");
                await conn.query(`INSERT INTO \`${config.table}\` (${insertColumns})
           VALUES (${insertPlaceholders})
           ON DUPLICATE KEY UPDATE ${updateColumns}`, values);
        ...
        if (syncMode === "full" || syncMode === "full_finalize") {
            if (incomingIds.length > 0) {
                await conn.query(`UPDATE \`${config.table}\`
           SET \`syncStatus\` = 'active',
               \`updatedBy\` = 'Google Sheets Sync',
               \`updateTimestamp\` = ?
           WHERE \`syncSource\` = 'google_sheets'
             AND \`${config.idColumn}\` IN (${incomingIds.map(() => "?").join(", ")})`, [syncTimestamp, ...incomingIds]);
                const [result] = await conn.query(`UPDATE \`${config.table}\`
           ...
           WHERE \`syncSource\` = 'google_sheets'
             ...
             AND COALESCE(NULLIF(TRIM(\`syncStatus\`), ''), 'active') <> 'removed'`, [syncTimestamp, ...incomingIds]);
           ...
           SET \`syncStatus\` = 'removed',
               ...
               \`updateTimestamp\` = ?
           WHERE \`syncSource\` = 'google_sheets'
             AND COALESCE(NULLIF(TRIM(\`syncStatus\`), ''), 'active') <> 'removed'`, [syncTimestamp]);
                removed = Number(result?.affectedRows || 0);
            }
        }
        await conn.commit();
    ...
    try {
      ...
      `);
        ...
        for (const row of orderRows) {
            ...
            if (!erp || !Number.isFinite(rate) || !isApproved)
                continue;
            const candidate = {
                erp,
                rate,
            ...
            const candidateWins = candidate.sortApprovedAt > existing.sortApprovedAt ||
...
async function getDeleteBlockers(db, tableName, id) {
    ...
    for (const ref of refs) {
        try {
            ...
            if (count > 0)
                blockers.push({ label: ref.label, count });
        }
        catch (err) {
...
async function dropRemovedFirmScopeColumns(db, database) {
    for (const tableName of REMOVED_FIRM_SCOPE_TABLES) {
        ...
        try {
            const [indexes] = await db.query("SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND INDEX_NAME = ?", [database, tableName, indexName]);
            if (indexes.length > 0) {
                await db.query(`ALTER TABLE \`${tableName}\` DROP INDEX \`${indexName}\``);
                console.log(`[DB] Dropped firm scope index ${indexName} on ${tableName}`);
            }
            const [columns] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = 'firmId'", [database, tableName]);
            if (columns.length > 0) {
...
async function ensureForeignKey(db, database, def) {
    try {
        const [rows] = await db.query("SELECT CONSTRAINT_NAME FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND CONSTRAINT_TYPE = 'FOREIGN KEY' AND CONSTRAINT_NAME = ?", [database, def.table, def.constraintName]);
        if (rows.length > 0)
            return;
        await ensureIndex(db, database, def.table, def.column, def.indexName);
        await db.query(`ALTER TABLE \`${def.table}\` ADD CONSTRAINT \`${def.constraintName}\` FOREIGN KEY (\`${def.column}\`) REFERENCES \`${def.refTable}\` (\`${def.refColumn}\`) ON DELETE RESTRICT ON UPDATE CASCADE`);
...
function mapSheetRowByHeaderMap_(row, headerMap, numericKeys = new Set()) {
    const mapped = {};
...
async function fetchLegacyItems(db) {
    ...
    LEFT JOIN (
      ...
      WHERE mi.status = 'Completed' AND mi.mrrType IN ('Rejection In', 'FG Purchase')
        AND NOT EXISTS (
          SELECT 1 FROM production_processing obsolete_pp
          WHERE obsolete_pp.id = mi.sourceTransactionId
            AND LOWER(TRIM(COALESCE(mi.sourceTransactionType, ''))) = 'production processing'
            AND LOWER(TRIM(COALESCE(mi.interFirmFlow, ''))) = 'yes'
            AND LOWER(TRIM(COALESCE(mi.autoGenerated, ''))) = 'yes'
            AND LOWER(TRIM(COALESCE(obsolete_pp.machineName, ''))) = 'corrugation liner'
        )
      GROUP BY jt.itemId
...
async function ensureColumnExists(db, database, table, column, type) {
    const [rows] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, table, column]);
    if (rows.length > 0)
        return;
    await db.query(`ALTER TABLE \`${table}\` ADD COLUMN \`${column}\` ${type}`);
}
async function fetchFirmWiseNpdItems(db, options) {
    let base;
    try {
        base = await fetchActiveNpdItems(db, { ...options, includeTotal: true, status: (options.status || "active") });
    }
    catch (error) {
    ...
    if (!unit1FirmId)
        console.warn("[NPD] Unit-I firm is missing from Firm Master; Corrugation Liner quantities will not be assigned.");
    if (!unit2FirmId)
        console.warn("[NPD] Unit-II firm is missing from Firm Master; Printing quantities will not be assigned.");
    ...
    const ensure = (itemId, firmId) => {
        const resolvedItemId = resolveStockItemId(itemId) || stringOrEmpty(itemId);
        if (!resolvedItemId || !firmId)
            return null;
        if (!stocks.has(resolvedItemId))
            stocks.set(resolvedItemId, new Map());
        const byFirm = stocks.get(resolvedItemId);
        if (!byFirm.has(firmId))
            byFirm.set(firmId, { opening: 0, receipt: 0, production: 0, invoiced: 0, tallyStock: null, tallyTimestamp: null, corrugation: 0 });
    ...
    if (itemIds.length) {
        ...
        try {
            ...
            const [receipts] = await db.query(`SELECT mi.firmId, mi.destinationFirmId, jt.itemId, jt.npdId, COALESCE(jt.invoiceQty, jt.qty, 0) qty FROM material_in mi JOIN JSON_TABLE(mi.lines, '$[*]' COLUMNS(itemId VARCHAR(36) PATH '$.itemId', npdId VARCHAR(36) PATH '$.npdId', qty DECIMAL(15,2) PATH '$.qty', invoiceQty DECIMAL(15,2) PATH '$.invoiceQty')) jt WHERE COALESCE(mi.status, '') NOT IN ('Cancelled', 'Canceled') AND mi.mrrType IN ('Rejection In', 'FG Purchase') AND NOT EXISTS (SELECT 1 FROM production_processing obsolete_pp WHERE obsolete_pp.id = mi.sourceTransactionId AND LOWER(TRIM(COALESCE(mi.sourceTransactionType, ''))) = 'production processing' AND LOWER(TRIM(COALESCE(mi.interFirmFlow, ''))) = 'yes' AND LOWER(TRIM(COALESCE(mi.autoGenerated, ''))) = 'yes' AND LOWER(TRIM(COALESCE(obsolete_pp.machineName, ''))) = 'corrugation liner')`);
...
async function dropConfirmedUnusedColumns(db, database, columns) {
    for (const candidate of columns) {
        const { table, column } = candidate;
        try {
            const [columnRows] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, table, column]);
            if (!columnRows.length) {
                console.log(`[DB] Deprecated column already absent: ${table}.${column}`);
                continue;
            }
            const [indexRows] = await db.query("SELECT DISTINCT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, table, column]);
            const [constraintRows] = await db.query("SELECT DISTINCT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, table, column]);
            const [viewRows] = await db.query("SELECT TABLE_NAME FROM information_schema.VIEWS WHERE TABLE_SCHEMA = ? AND VIEW_DEFINITION LIKE ? LIMIT 1", [database, `%${table}%${column}%`]);
            const [triggerRows] = await db.query("SELECT TRIGGER_NAME FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = ? AND ACTION_STATEMENT LIKE ? LIMIT 1", [database, `%${column}%`]);
            const dependencies = [
                ...indexRows.map((row) => `index ${row.INDEX_NAME}`),
                ...constraintRows.map((row) => `constraint ${row.CONSTRAINT_NAME}`),
                ...viewRows.map((row) => `view ${row.TABLE_NAME}`),
                ...triggerRows.map((row) => `trigger ${row.TRIGGER_NAME}`),
            ];
            ...
            console.warn(`[DB] Could not clean deprecated column ${table}.${column}:`, error.message);
        }
    }
}
const INTER_FIRM_COLUMNS = [
    ...
    ["autoGenerated", "VARCHAR(10)"], ["mrrSource", "VARCHAR(30)"], ["linkedGateEntryId", "VARCHAR(36)"],
    ["linkedMrrId", "VARCHAR(36)"], ["linkedInvoiceId", "VARCHAR(36)"],
];
async function ensureInterFirmSchema(db, database) {
    await db.query(`CREATE TABLE IF NOT EXISTS inter_firm_pending_invoices (
    id VARCHAR(36) PRIMARY KEY, firmId VARCHAR(36), orderFirmId VARCHAR(36),
    sourceFirmId VARCHAR(36) NOT NULL, destinationFirmId VARCHAR(36) NOT NULL,
    ...
    for (const table of ["orders", "productions", "production_processing", "gate_entries", "material_in", "material_issues", "invoices", "invoice_line_items"]) {
        for (const [column, type] of INTER_FIRM_COLUMNS) {
            try {
                ...
                console.warn(`[DB] Could not ensure ${table}.${column}:`, error.message);
            }
        }
    }
    for (const table of ["gate_entries", "material_in"]) {
        try {
            await ensureColumnExists(db, database, table, "firmName", "VARCHAR(255)");
        }
        catch { }
    }
    ...
    for (const [column, type] of [["routeSourceFirmId", "VARCHAR(36)"], ["routeDestinationFirmId", "VARCHAR(36)"], ["routeSequence", "INT NOT NULL DEFAULT 1"], ["routeActive", "VARCHAR(10) NOT NULL DEFAULT 'No'"]]) {
        try {
            ...
            console.warn(`[DB] Could not ensure firms.${column}:`, error.message);
        }
    }
...
async function ensureInternalFirmSuppliers(db, database) {
    ...
    const [firmRows] = await db.query("SELECT id, firmName FROM `firms` WHERE LOWER(firmName) LIKE '%unit%' ORDER BY firmName");
    const firms = firmRows.filter((firm) => {
        const normalized = String(firm.firmName || "").toLowerCase().replace(/[^a-z0-9]/g, "");
        return normalized.includes("laxminarayan") && (normalized.endsWith("uniti") || normalized.endsWith("unitii"));
    });
    for (const firm of firms) {
        const firmId = String(firm.id || "").trim();
        const firmName = String(firm.firmName || "").trim();
        if (!firmId || !firmName)
            continue;
        const [supplierRows] = await db.query("SELECT id FROM `suppliers` WHERE firmId = ? LIMIT 1", [firmId]);
        let supplierId = String(supplierRows[0]?.id || "").trim();
        if (!supplierId) {
            const [nameRows] = await db.query("SELECT id FROM `suppliers` WHERE LOWER(TRIM(name)) = LOWER(TRIM(?)) LIMIT 1", [firmName]);
            supplierId = String(nameRows[0]?.id || "").trim();
        }
        if (!supplierId) {
            supplierId = crypto.randomUUID();
            await db.query("INSERT INTO suppliers (id, name, firmId, gstSupplyType, active, updatedBy, updateTimestamp) VALUES (?, ?, ?, 'INTRA_STATE', 'Yes', 'System', ?)", [supplierId, firmName, firmId, new Date().toISOString()]);
        }
        else {
            await db.query("UPDATE suppliers SET firmId = ?, name = ?, active = 'Yes', updatedBy = 'System', updateTimestamp = ? WHERE id = ?", [firmId, firmName, new Date().toISOString(), supplierId]);
        }
    }
    const [autoRows] = await db.query("SELECT id, supplierId, sourceFirmId FROM gate_entries WHERE autoGenerated = 'Yes' AND interFirmFlow = 'Yes' AND sourceFirmId IS NOT NULL AND sourceFirmId <> ''");
    for (const row of autoRows) {
        const [supplierRows] = await db.query("SELECT id FROM suppliers WHERE firmId = ? LIMIT 1", [String(row.sourceFirmId || "")]);
        const supplierId = String(supplierRows[0]?.id || "").trim();
        if (supplierId && String(row.supplierId || "") !== supplierId)
            await db.query("UPDATE gate_entries SET supplierId = ? WHERE id = ?", [supplierId, row.id]);
    }
    const [autoMrrRows] = await db.query("SELECT id, supplierId, sourceFirmId FROM material_in WHERE autoGenerated = 'Yes' AND interFirmFlow = 'Yes' AND sourceFirmId IS NOT NULL AND sourceFirmId <> ''");
    for (const row of autoMrrRows) {
        const [supplierRows] = await db.query("SELECT id FROM suppliers WHERE firmId = ? LIMIT 1", [String(row.sourceFirmId || "")]);
        const supplierId = String(supplierRows[0]?.id || "").trim();
        if (supplierId && String(row.supplierId || "") !== supplierId)
...
async function backfillSystemGeneratedMrrApprovals(db) {
    const fallbackTimestamp = new Date().toISOString();
    const systemEmail = "system@lngrp.in";
    const systemRemark = "Auto-approved: system-generated MRR";
    await db.query(`UPDATE material_in
     SET
       phTimestamp = COALESCE(NULLIF(TRIM(phTimestamp), ''), NULLIF(TRIM(updateTimestamp), ''), NULLIF(TRIM(timestamp), ''), ?),
       phEmailId = COALESCE(NULLIF(TRIM(phEmailId), ''), ?),
       plant_head_remark = COALESCE(NULLIF(TRIM(plant_head_remark), ''), ?),
       accTimestamp = COALESCE(NULLIF(TRIM(accTimestamp), ''), NULLIF(TRIM(updateTimestamp), ''), NULLIF(TRIM(timestamp), ''), ?),
       accEmailId = COALESCE(NULLIF(TRIM(accEmailId), ''), ?),
       accounts_remark = COALESCE(NULLIF(TRIM(accounts_remark), ''), ?),
       mdTimestamp = COALESCE(NULLIF(TRIM(mdTimestamp), ''), NULLIF(TRIM(updateTimestamp), ''), NULLIF(TRIM(timestamp), ''), ?),
       mdEmailId = COALESCE(NULLIF(TRIM(mdEmailId), ''), ?),
       md_approval_remark = COALESCE(NULLIF(TRIM(md_approval_remark), ''), ?),
       status = CASE WHEN NULLIF(TRIM(tallyTimestamp), '') IS NOT NULL THEN 'Completed' ELSE 'Pending Tally' END
     WHERE LOWER(TRIM(COALESCE(autoGenerated, ''))) = 'yes'
        OR LOWER(TRIM(COALESCE(mrrSource, ''))) = 'system generated'`, [
        fallbackTimestamp, systemEmail, systemRemark,
        fallbackTimestamp, systemEmail, systemRemark,
        fallbackTimestamp, systemEmail, systemRemark,
    ]);
}
...
async function generateFirmDocumentNo(db, firmId, documentCode, dateStr) {
    const fy = getShortFinancialYear(dateStr);
    if (!fy)
        throw new Error("Could not generate document number because date is invalid.");
    ...
    await db.query(`INSERT INTO transaction_counters (counterKey, lastValue)
     VALUES (?, 0)
     ON DUPLICATE KEY UPDATE lastValue = GREATEST(lastValue, VALUES(lastValue))`, [key]);
...
async function generateLockedGateEntryNo(db, dateStr, firmId) {
    return generateFirmDocumentNo(db, firmId, "GE", dateStr);
}
...
async function automateInterFirmProduction(db, sourceId, sourceType = "Production", providedConnection) {
    ...
    try {
        ...
        if (sourceType === "Production Processing") {
            const [processingRows] = await conn.query("SELECT * FROM `production_processing` WHERE id = ? LIMIT 1 FOR UPDATE", [sourceId]);
            sourceRecord = processingRows[0] || null;
            productionId = String(sourceRecord?.productionId || "");
        }
        const eligiblePrintingRecord = sourceType === "Production Processing" &&
            normalizeMachineName(String(sourceRecord?.machineName || "")) === "Printing" &&
            String(sourceRecord?.completionStatus || "").trim().toLowerCase() === "full";
        if (!eligiblePrintingRecord) {
            await skip();
            return;
        }
        ...
        if (sourceType === "Production Processing" && normalizedMachine === "Printing") {
            if (!unit2?.id || !lnki?.id) {
        ...
        let interFirmRatePercent = Number.isFinite(configuredInterFirmRate) && configuredInterFirmRate >= 0 && configuredInterFirmRate <= 100
            ...
            : 100;
        try {
            const pairRatesRaw = interFirmSettingRows[0]?.interFirmPairRates;
            const pairs = JSON.parse(String(pairRatesRaw || "[]"));
            const pair = Array.isArray(pairs) && pairs.find((row) => String(row?.fromFirmId) === String(sourceFirmId) && String(row?.toFirmId) === String(destinationFirmId));
            if (pair && Number.isFinite(Number(pair.rate)) && Number(pair.rate) >= 0 && Number(pair.rate) <= 100)
                interFirmRatePercent = Number(pair.rate);
            else if (pairRatesRaw !== null && pairRatesRaw !== undefined)
                interFirmRatePercent = 100;
        }
        ...
        const baseRate = Number.isFinite(customerOrderRate) && customerOrderRate > 0
            ? customerOrderRate
            : Number(sourceRecord.rate || production.rate || 0);
        const rate = Number((baseRate * interFirmRatePercent / 100).toFixed(2));
       ...
       ORDER BY ip.updateTimestamp DESC LIMIT 2`, [sourceTransactionType, productionId, sourceFirmId, destinationFirmId]);
        ...
        if (!sourceIssue) {
            ...
            const issueDate = String(sourceRecord.date || production.date || now.slice(0, 10));
            const issueNo = await generateSimpleTransactionNumber(conn, "material_issues", "issueNo", "MIS", issueDate);
            const issueLineId = crypto.randomUUID();
            const amount = Number((qty * rate).toFixed(2));
            await conn.query(`INSERT INTO material_issues (id, issueNo, date, issueType, firmId, remarks, interFirmFlow, autoGenerated, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, updatedBy, updateTimestamp)
         VALUES (?, ?, ?, 'Without Job', ?, ?, 'Yes', 'Yes', ?, ?, ?, ?, 'System', ?)`, [issueId, issueNo, issueDate, sourceFirmId, `Inter-firm transfer to ${destinationFirmId}`, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, now]);
            await conn.query(`INSERT INTO material_issue_lines (id, materialIssueId, materialId, qty, uom, lastPurchaseRate, openingRate, rate, amount, updatedBy, updateTimestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'System', ?)`, [issueLineId, issueId, itemId, qty, sourceRecord.uom || production.uom || "", rate, rate, rate, amount, now]);
            sourceIssue = { id: issueId, issueNo };
        }
        const [internalSupplierRows] = await conn.query("SELECT id FROM `suppliers` WHERE firmId = ? LIMIT 1", [sourceFirmId]);
        const internalSupplierId = String(internalSupplierRows[0]?.id || "").trim();
        if (!internalSupplierId) {
            throw new Error(`Internal supplier mapping is missing for source firm ${sourceFirmId}.`);
        }
        ...
        if (!mrr) {
            ...
            // Use the same transaction connection so counter locking and the MRR
            // insert commit atomically with the pending invoice and gate entry.
            const transactionNo = await generateLockedMrrNo(conn, String(sourceRecord.date || production.date || now.slice(0, 10)), destinationFirmId);
            await conn.query(`INSERT INTO material_in (id, firmId, transactionNo, mrrType, timestamp, entryEmailId, date, invoiceNo, supplierId, totalPoValue, totalInvoiceValue, totalActualValue, totalInvoiceValueAfterGst, totalAmount, \`lines\`, status, mrrSource, autoGenerated, interFirmFlow, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, linkedInvoiceId, updatedBy, updateTimestamp) VALUES (?, ?, ?, 'FG Purchase', ?, 'system', ?, '', ?, ?, ?, ?, ?, ?, ?, 'Pending Tally', 'System Generated', 'Yes', 'Yes', ?, ?, ?, ?, NULL, 'System', ?)`, [mrrId, destinationFirmId, transactionNo, now, String(sourceRecord.date || production.date || now.slice(0, 10)), internalSupplierId, qty * rate, qty * rate, qty * rate, qty * rate * (1 + gstRate / 100), qty * rate * (1 + gstRate / 100), JSON.stringify([{ itemId, qty, rate, gstRate }]), sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, now]);
            await conn.query(`UPDATE material_in
         SET phTimestamp = ?, phEmailId = ?, plant_head_remark = ?,
             accTimestamp = ?, accEmailId = ?, accounts_remark = ?,
             mdTimestamp = ?, mdEmailId = ?, md_approval_remark = ?
         WHERE id = ?`, [
                now, "system@lngrp.in", "Auto-approved: system-generated MRR",
                now, "system@lngrp.in", "Auto-approved: system-generated MRR",
                now, "system@lngrp.in", "Auto-approved: system-generated MRR",
                mrrId,
            ]);
        ...
        if (!gate) {
            ...
            const gateDate = String(sourceRecord.date || production.date || now.slice(0, 10));
            const gateEntryNo = await generateLockedGateEntryNo(conn, gateDate);
            await conn.query(`INSERT INTO gate_entries (id, gateEntryNo, firmId, date, supplierId, purpose, invoiceNo, invoiceValue, truckNo, mrrId, mrrDate, mrrNo, status, autoGenerated, interFirmFlow, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, updatedBy, updateTimestamp) VALUES (?, ?, ?, ?, ?, 'Material Receipt', '', ?, '', ?, ?, ?, 'Active', 'Yes', 'Yes', ?, ?, ?, ?, 'System', ?)`, [gateId, gateEntryNo, destinationFirmId, gateDate, internalSupplierId, qty * rate, mrr.id, gateDate, mrr.transactionNo, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, now]);
...
async function backfillInterFirmProductionProcessing(db) {
    ...
    WHERE LOWER(TRIM(COALESCE(pp.completionStatus, ''))) = 'full'
      AND LOWER(TRIM(COALESCE(pp.machineName, ''))) = 'printing'
...
const CORRUGATION_OPERATIONAL_EXCLUSION_TABLES = new Set([
    "material_issues",
    "inter_firm_pending_invoices",
    "material_in",
    "gate_entries",
]);
async function excludeLegacyCorrugationInterFirmRows(db, tableName, rows) {
    if (!CORRUGATION_OPERATIONAL_EXCLUSION_TABLES.has(tableName) || !Array.isArray(rows) || rows.length === 0)
        return rows;
    const candidates = rows.filter((row) => {
        const isProductionProcessing = String(row?.sourceTransactionType || "").trim().toLowerCase() === "production processing";
        const isInterFirm = String(row?.interFirmFlow || "").trim().toLowerCase() === "yes";
        const isAutoGenerated = tableName === "inter_firm_pending_invoices" || String(row?.autoGenerated || "").trim().toLowerCase() === "yes";
        return isProductionProcessing && isInterFirm && isAutoGenerated && String(row?.sourceTransactionId || "").trim();
    });
    if (candidates.length === 0)
        return rows;
    const sourceIds = Array.from(new Set(candidates.map((row) => String(row.sourceTransactionId).trim())));
    const [processingRows] = await db.query(`SELECT id FROM \`production_processing\`
     WHERE id IN (${sourceIds.map(() => "?").join(",")})
       AND LOWER(TRIM(COALESCE(machineName, ''))) = 'corrugation liner'`, sourceIds);
    const obsoleteSourceIds = new Set(processingRows.map((row) => String(row.id || "").trim()));
    if (obsoleteSourceIds.size === 0)
        return rows;
    return rows.filter((row) => !obsoleteSourceIds.has(String(row?.sourceTransactionId || "").trim()));
}
...
async function syncCorrugationLinerOutput(db, processingId) {
    ...
    if (!row || normalizeMachineName(String(row.machineName || "")) !== "Corrugation Liner" ||
        String(row.completionStatus || "").trim().toLowerCase() !== "full")
...
async function backfillCorrugationLinerOutput(db) {
    ...
    let skipped = 0;
    for (const row of rows) {
        if (seen.has(String(row.productionId)))
            continue;
        seen.add(String(row.productionId));
        const result = await syncCorrugationLinerOutput(db, String(row.id));
        if (result.updated)
            updated++;
        else
            skipped++;
...
async function buildMaterialValuationMaps(db) {
    ...
    const openingRateByMaterial = new Map(materialRows.map((row) => [String(row.id || ""), roundCurrency(Number(row.openingRate || 0))]));
    const [itemRows] = await db.query("SELECT id, rate, consumable FROM `items`");
    const npdRateByMaterialId = new Map();
    for (const item of itemRows) {
        if (!isTruthyDbValue(item.consumable))
            continue;
    ...
    const latestPurchaseByMaterial = new Map();
    ...
    LEFT JOIN \`materials\` m ON m.id = ps.materialId
  `);
    const receiptRateByPackingSlip = new Map();
    for (const slip of packingSlipRows) {
        ...
        if (rate > 0)
            receiptRateByPackingSlip.set(String(slip.id || ""), rate);
    }
...
async function backfillNonJobMaterialIssueValuation(db) {
    ...
    const [materialInRows] = await db.query("SELECT id, date, timestamp, `lines` FROM `material_in`");
    const purchaseRatesByMaterial = new Map();
    for (const receipt of materialInRows) {
    ...
    const getLatestPurchaseRateAsOf = (materialId, issueDate) => {
        ...
        if (!issueDateTime)
            return 0;
        return ((purchaseRatesByMaterial.get(materialId) || [])
  ...
  `);
    ...
    for (const line of lineRows) {
        ...
        if (finalRate <= 0 && finalAmount <= 0 && finalOpeningRate <= 0 && finalLastPurchaseRate <= 0)
            continue;
        await db.query("UPDATE `material_issue_lines` SET `lastPurchaseRate` = ?, `openingRate` = ?, `rate` = ?, `amount` = ? WHERE `id` = ? AND (COALESCE(`rate`, 0) = 0 OR COALESCE(`amount`, 0) = 0 OR COALESCE(`lastPurchaseRate`, 0) = 0 OR COALESCE(`openingRate`, 0) = 0)", [finalLastPurchaseRate, finalOpeningRate, finalRate, finalAmount, line.id]);
        updatedCount += 1;
    }
    if (updatedCount > 0) {
...
async function backfillOldJobMaterialIssueReturnValuation(db) {
  ...
  `);
    ...
    const issueReelsByLine = new Map();
    ...
    for (const line of issueLineRows) {
        ...
        if (finalRate <= 0 && finalAmount <= 0 && finalOpeningRate <= 0 && finalLastPurchaseRate <= 0)
            continue;
        await db.query("UPDATE `material_issue_lines` SET `lastPurchaseRate` = ?, `openingRate` = ?, `rate` = ?, `amount` = ? WHERE `id` = ? AND (COALESCE(`lastPurchaseRate`, 0) = 0 OR COALESCE(`openingRate`, 0) = 0 OR COALESCE(`rate`, 0) = 0 OR COALESCE(`amount`, 0) = 0)", [finalLastPurchaseRate, finalOpeningRate, finalRate, finalAmount, line.id]);
        issueUpdatedCount += 1;
    }
  ...
  `);
    ...
    const returnReelsByLine = new Map();
    ...
    for (const line of returnLineRows) {
        ...
        const issueValue = (productionId ? issueValueByJobMaterial.get(`pid:${productionId}::${materialId}`) : undefined) ||
        ...
        if (finalRate <= 0 && finalAmount <= 0 && finalOpeningRate <= 0 && finalLastPurchaseRate <= 0)
            continue;
        await db.query("UPDATE `material_return_lines` SET `lastPurchaseRate` = ?, `openingRate` = ?, `rate` = ?, `amount` = ? WHERE `id` = ? AND (COALESCE(`lastPurchaseRate`, 0) = 0 OR COALESCE(`openingRate`, 0) = 0 OR COALESCE(`rate`, 0) = 0 OR COALESCE(`amount`, 0) = 0)", [finalLastPurchaseRate, finalOpeningRate, finalRate, finalAmount, line.id]);
...
async function ensureAuditColumnsForAllTables(db, database) {
    const [rows] = await db.query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_TYPE = 'BASE TABLE'", [database]);
    for (const row of rows) {
...
async function applyTruckStatusUpdate(db, input) {
    ...
    await db.query("UPDATE `trucks` SET `liveStatus` = ?, `statusUpdatedAt` = ?, `statusUpdatedBy` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE id = ?", [liveStatus, now, actor, actor, now, truckId]);
    const logId = crypto.randomUUID();
    await db.query("INSERT INTO `truck_status_logs` (`id`, `truckId`, `truckNo`, `liveStatus`, `statusUpdatedAt`, `statusUpdatedBy`, `updateSource`, `sourceRefType`, `sourceRefId`, `invoiceNo`, `partyName`, `driverName`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [logId, truckId, truckNo, liveStatus, now, actor, input.updateSource, sourceRefType, sourceRefId, invoiceNo, partyName, driverName, actor, now]);
...
async function getTruckActivePartyName(db, truckId) {
    ...
    if (companyIds.size) {
        ...
        const [companyRows] = await db.query(`SELECT id, name FROM \`companies\` WHERE id IN (${ids.map(() => "?").join(",")})`, ids);
...
async function ensureGatePassNullableColumns(db, database) {
    ...
    for (const { column, type } of nullableColumns) {
        const [rows] = await db.query("SELECT IS_NULLABLE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, "gate_passes", column]);
        const metadata = rows[0];
        if (!metadata)
            continue;
        if (String(metadata.IS_NULLABLE || "").toUpperCase() === "YES")
            continue;
...
async function getExistingColumnNames(db, database, table) {
    const [rows] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", [database, table]);
...
async function ensureUsersCanonicalData(db, database) {
    ...
    if (columns.has("userId")) {
        ...
        if (userIdSources.length > 0) {
            ...
            await db.query(`UPDATE \`users\` SET \`userId\` = COALESCE(NULLIF(TRIM(\`userId\`), ''), ${expr}) WHERE COALESCE(NULLIF(TRIM(\`userId\`), ''), '') = ''`);
        }
    }
    if (columns.has("updatedBy")) {
        ...
        if (updatedBySources.length > 0) {
            ...
            await db.query(`UPDATE \`users\` SET \`updatedBy\` = COALESCE(NULLIF(TRIM(\`updatedBy\`), ''), ${expr}) WHERE COALESCE(NULLIF(TRIM(\`updatedBy\`), ''), '') = ''`);
        }
    }
    if (columns.has("updateTimestamp")) {
        ...
        if (updateTimestampSources.length > 0) {
            ...
            await db.query(`UPDATE \`users\` SET \`updateTimestamp\` = COALESCE(NULLIF(TRIM(\`updateTimestamp\`), ''), ${expr}) WHERE COALESCE(NULLIF(TRIM(\`updateTimestamp\`), ''), '') = ''`);
        }
    }
    if (columns.has("menuAccess") && columns.has("menuaccess")) {
        await db.query("UPDATE `users` SET `menuAccess` = COALESCE(`menuAccess`, `menuaccess`) WHERE `menuAccess` IS NULL");
    }
}
...
async function migrateLinkedTableToNpd(db, database, table, itemToNpdId) {
    ...
    for (const row of rows) {
        ...
        if (currentNpdId === resolvedNpdId && currentLegacyId === resolvedNpdId)
            continue;
...
async function migrateMaterialInLinesToNpd(db, itemToNpdId) {
    ...
    for (const row of rows) {
        ...
        const nextLines = rawLines.map((line) => {
            ...
            if (currentNpdId === resolvedNpdId && currentItemId === resolvedNpdId)
                return line;
            changed = true;
            migrated += 1;
            return {
...
async function ensureIndianStatesSeed(db) {
    ...
    for (const name of INDIA_STATES_AND_UNION_TERRITORIES) {
        ...
        if (existingNames.has(normalizedName))
            continue;
        await db.query("INSERT INTO `states` (`id`, `name`, `active`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, ?, ?)", [crypto.randomUUID(), name, "Yes", "System Seed", timestamp]);
...
async function ensureReelTransferRuntimeSchema(db) {
    const [databaseRows] = await db.query("SELECT DATABASE() AS `databaseName`");
    const database = String(databaseRows[0]?.databaseName || "").trim();
    if (!database)
        throw new Error("Could not identify the active database for reel transfer setup.");
    const columns = [
...
function normalizeWorkflowStatus(tableName, row) {
    ...
    if (tableName === "material_in") {
        const isSystemGenerated = String(normalized.autoGenerated || "").trim().toLowerCase() === "yes" ||
            String(normalized.mrrSource || "").trim().toLowerCase() === "system generated";
        if (isSystemGenerated) {
            const approvalTimestamp = String(normalized.updateTimestamp || normalized.timestamp || new Date().toISOString()).trim();
            const systemEmail = "system@lngrp.in";
            const systemRemark = "Auto-approved: system-generated MRR";
            normalized.phTimestamp = normalized.phTimestamp || approvalTimestamp;
            normalized.phEmailId = normalized.phEmailId || systemEmail;
            normalized.plant_head_remark = normalized.plant_head_remark || systemRemark;
            normalized.accTimestamp = normalized.accTimestamp || approvalTimestamp;
            normalized.accEmailId = normalized.accEmailId || systemEmail;
            normalized.accounts_remark = normalized.accounts_remark || systemRemark;
            normalized.mdTimestamp = normalized.mdTimestamp || approvalTimestamp;
            normalized.mdEmailId = normalized.mdEmailId || systemEmail;
            normalized.md_approval_remark = normalized.md_approval_remark || systemRemark;
            normalized.status = normalized.tallyTimestamp ? "Completed" : "Pending Tally";
            return normalized;
        }
        if (normalized.tallyTimestamp)
            normalized.status = "Completed";
        else if (normalized.mdTimestamp)
            normalized.status = "Pending Tally";
        else if (normalized.accTimestamp)
            normalized.status = "Pending MD";
        else if (normalized.phTimestamp)
            normalized.status = "Pending Accounts";
        else
            normalized.status = "Pending PH";
        return normalized;
    }
    if (tableName === "productions") {
        if (currentStatus === "Completed" || normalized.tallyTimestamp)
            normalized.status = "Completed";
    ...
    if (tableName === "consumptions") {
        if (normalized.tallyTimestamp)
            normalized.status = "Completed";
        else if (normalized.cancelTimestamp)
            normalized.status = "Cancelled";
        else if (normalized.phTimestamp)
            normalized.status = "Pending Tally";
        else
...
function normalizeMachineName(name) {
    ...
    if (trimmed === "Corrugation Finger")
        return "Corrugation Paper";
    if (trimmed === "Corrugation Linear")
        return "Corrugation Liner";
    if (trimmed === "Stithing")
        return "Stitching";
    return trimmed;
}
...
async function ensureDevSeedUser(db) {
    ...
    if (existing?.id) {
        await db.query("UPDATE `users` SET `role` = 'Admin', `status` = 'Active', `menuAccess` = ?, `password` = ? WHERE `id` = ?", [menuAccess, password, existing.id]);
        return;
    }
...
async function ensureUniqueMaterialErpIndex(db, database) {
    const indexName = "uq_materials_erpCode";
    const [indexes] = await db.query("SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'materials' AND INDEX_NAME = ?", [database, indexName]);
    if (indexes.length > 0)
        return;
...
async function ensureUniquePackingSlipReelNoIndex(db, database) {
    ...
    try {
        const [indexes] = await db.query("SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'material_in_packing_slips' AND INDEX_NAME = ?", [database, indexName]);
        if (indexes.length > 0)
            return;
        const [duplicates] = await db.query(`
...
async function ensureUniqueMachineNameIndex(db, database) {
    ...
    const [indexes] = await db.query("SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'machines' AND INDEX_NAME = ?", [database, indexName]);
    if (indexes.length > 0)
        return;
...
async function generateSimpleTransactionNumber(db, tableName, columnName, prefix, dateStr) {
    const fy = getShortFinancialYear(dateStr);
    if (!fy)
        throw new Error(`Could not generate ${columnName} because date is invalid.`);
    const likePattern = `${prefix}/${fy}/%`;
    const [rows] = await db.query(`SELECT \`${columnName}\` AS serialValue FROM \`${tableName}\` WHERE \`${columnName}\` LIKE ? ORDER BY CAST(SUBSTRING_INDEX(\`${columnName}\`,'/',-1) AS UNSIGNED) DESC LIMIT 1`, [likePattern]);
    let lastNum = 0;
...
function normalizeProductionJobNumber(value, dateValue) {
...
async function generateProductionJobNumber(db, dateValue) {
    const fy = getShortFinancialYear(String(dateValue || "")) || getShortFinancialYear();
    const [rows] = await db.query("SELECT transactionNo FROM `productions` WHERE transactionNo LIKE ? ORDER BY CAST(SUBSTRING_INDEX(transactionNo, '/', -1) AS UNSIGNED) DESC LIMIT 1", [`JOB/${fy}/%`]);
...
async function backfillMissingScheduleNos(db) {
    const [rows] = await db.query(`SELECT \`id\`, \`scheduledDate\`
...
async function generateLoadingSlipNo(db, tableName, dateStr) {
    ...
    const [rows] = await db.query(`SELECT \`slipNo\` AS serialValue FROM \`${tableName}\` WHERE \`slipNo\` LIKE ? ORDER BY CAST(SUBSTRING_INDEX(\`slipNo\`,'/',-1) AS UNSIGNED) DESC LIMIT 1`, [likePattern]);
    let lastNum = 0;
    const latestValue = String((rows[0] || {}).serialValue || "").trim();
    if (latestValue) {
        const suffix = latestValue.split("/").pop() || "0";
...
async function generateDynamicInvoiceNo(db, dateStr, firmId) {
    ...
    for (const row of invoiceRows) {
        const invoiceNo = String(row?.invoiceNo || "").trim();
        const expectedPrefix = sequencePrefixes
            .map((sequencePrefix) => `${sequencePrefix}${separator}${fy}${separator}`)
            .find((candidate) => invoiceNo.startsWith(candidate));
        if (!expectedPrefix)
            continue;
...
app.get("/api/db-status", async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            error: error.message,
            code: error.code,
            hint: error.code === 'ECONNREFUSED' ? "Connection refused. Check if the database host and port are correct and if remote access is allowed." :
                error.code === 'ETIMEDOUT' ? "Connection timed out. Check DB_HOST/DB_PORT, MySQL remote access, and firewall allowlist." :
                    error.code === 'ER_ACCESS_DENIED_ERROR' ? "Access denied. Check your username and password." :
                        error.code === 'ENOTFOUND' ? "Database host not found. Check the hostname." :
...
async function initDb(retries = 5) {
    for (let i = 0; i < retries; i++) {
        ...
        if (!db || !database) {
            if (i === retries - 1)
                console.warn("[DB] Skipping tables initialization (no credentials or database name)");
            return;
        }
        try {
      ...
      `);
            ...
            try {
                const [columns] = await db.query("SELECT IS_NULLABLE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'purchase_orders' AND COLUMN_NAME = 'indentId'", [database]);
      ...
      `);
            await db.query(`
        CREATE TABLE IF NOT EXISTS \`reel_transfers\` (
          \`id\` VARCHAR(36) PRIMARY KEY,
          \`transferNo\` VARCHAR(100) NOT NULL UNIQUE,
          \`date\` VARCHAR(50) NOT NULL,
          \`sourceProductionId\` VARCHAR(36) NOT NULL,
          \`sourceJobNo\` VARCHAR(100) NOT NULL,
          \`targetProductionId\` VARCHAR(36) NOT NULL,
          \`targetJobNo\` VARCHAR(100) NOT NULL,
          \`materialReturnId\` VARCHAR(36) NOT NULL,
          \`materialIssueId\` VARCHAR(36) NOT NULL,
          \`remarks\` TEXT,
          \`totalWeightKg\` DECIMAL(15,2) NOT NULL DEFAULT 0,
          \`totalAmount\` DECIMAL(15,2) NOT NULL DEFAULT 0,
          \`updatedBy\` VARCHAR(255),
          \`updateTimestamp\` VARCHAR(255)
        )
      ...
      `);
        ...
        CREATE TABLE IF NOT EXISTS \`reel_transfer_lines\` (
          ...
          \`rate\` DECIMAL(15,2) NOT NULL DEFAULT 0,
          \`amount\` DECIMAL(15,2) NOT NULL DEFAULT 0,
          \`materialReturnReelLineId\` VARCHAR(36) NOT NULL,
          \`materialIssueReelLineId\` VARCHAR(36) NOT NULL,
          \`updatedBy\` VARCHAR(255),
          \`updateTimestamp\` VARCHAR(255)
        )
      `);
            // These workflow tables used to be present only in older databases. Keep
            // their complete base schemas here so a fresh database can serve the
            // generic entity APIs before any additive migrations run below.
            await db.query(`
      ...
      `);
            try {
                ...
                if (existingServiceCount === 0) {
                    const timestamp = new Date().toISOString();
                    await db.query("INSERT INTO `services` (`id`,`name`,`active`,`updatedBy`,`updateTimestamp`) VALUES (UUID(),'Repairing','Yes','System User',?), (UUID(),'Other Charges','Yes','System User',?)", [timestamp, timestamp]);
                }
            }
            catch (seedError) {
      ...
      `);
            ...
            try {
                const [columns] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'npd'", [database]);
                const existingColumns = columns.map((c) => c.COLUMN_NAME);
      ...
      `);
            ...
            const migrations = [
                ...
                { table: "material_issues", column: "firmId", type: "VARCHAR(36)" },
                ...
                { table: "material_returns", column: "firmId", type: "VARCHAR(36)" },
                ...
                { table: "reel_transfers", column: "id", type: "VARCHAR(36)" },
                { table: "reel_transfers", column: "transferNo", type: "VARCHAR(100)" },
                { table: "reel_transfers", column: "date", type: "VARCHAR(50)" },
                { table: "reel_transfers", column: "sourceProductionId", type: "VARCHAR(36)" },
                { table: "reel_transfers", column: "sourceJobNo", type: "VARCHAR(100)" },
                { table: "reel_transfers", column: "targetProductionId", type: "VARCHAR(36)" },
                { table: "reel_transfers", column: "targetJobNo", type: "VARCHAR(100)" },
                { table: "reel_transfers", column: "materialReturnId", type: "VARCHAR(36)" },
                { table: "reel_transfers", column: "materialIssueId", type: "VARCHAR(36)" },
                { table: "reel_transfers", column: "remarks", type: "TEXT" },
                { table: "reel_transfers", column: "totalWeightKg", type: "DECIMAL(15,2) NOT NULL DEFAULT 0" },
                { table: "reel_transfers", column: "totalAmount", type: "DECIMAL(15,2) NOT NULL DEFAULT 0" },
                { table: "reel_transfers", column: "updatedBy", type: "VARCHAR(255)" },
                { table: "reel_transfers", column: "updateTimestamp", type: "VARCHAR(255)" },
                { table: "reel_transfer_lines", column: "id", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "reelTransferId", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "materialId", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "packingSlipId", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "ourReelNo", type: "VARCHAR(100)" },
                { table: "reel_transfer_lines", column: "weightKg", type: "DECIMAL(15,2) NOT NULL DEFAULT 0" },
                { table: "reel_transfer_lines", column: "rate", type: "DECIMAL(15,2) NOT NULL DEFAULT 0" },
                { table: "reel_transfer_lines", column: "amount", type: "DECIMAL(15,2) NOT NULL DEFAULT 0" },
                { table: "reel_transfer_lines", column: "materialReturnReelLineId", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "materialIssueReelLineId", type: "VARCHAR(36)" },
                { table: "reel_transfer_lines", column: "updatedBy", type: "VARCHAR(255)" },
                { table: "reel_transfer_lines", column: "updateTimestamp", type: "VARCHAR(255)" },
                ...
                { table: "invoice_line_items", column: "loadingSlipId", type: "VARCHAR(36)" },
                ...
                { table: "materials", column: "gstRate", type: "DECIMAL(5,2) NOT NULL DEFAULT 0" },
            ...
            for (const m of migrations) {
                try {
                    ...
                    const [columns] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, m.table, m.column]);
                    if (columns.length === 0) {
            ...
            await backfillSystemGeneratedMrrApprovals(db);
        ...
        `);
            ...
            try {
                ...
                console.warn("[DB] Could not backfill production job numbers:", err.message);
            }
            try {
                const [oldLeastSheetWeight] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, "productions", "leastSheetWeight"]);
                const [newLeastGsm] = await db.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?", [database, "productions", "leastGsm"]);
            ...
            try {
                const [rows] = await db.query("SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'invoice_line_items' AND CONSTRAINT_NAME = 'fk_invoice_line_items_itemId_items' AND REFERENCED_TABLE_NAME = 'items'");
            ...
            try {
                const [rows] = await db.query("SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'invoice_line_items' AND CONSTRAINT_NAME = 'fk_invoice_line_items_itemId_npd' AND REFERENCED_TABLE_NAME = 'npd'");
                if (Array.isArray(rows) && rows.length > 0) {
...
async function fetchOpenPhysicalStockSession(db) {
    const [rows] = await db.query("SELECT * FROM `physical_stock_sessions` WHERE LOWER(TRIM(`status`)) = 'open' ORDER BY `startedAt` DESC LIMIT 1");
    return (rows[0] || null);
}
async function generatePhysicalStockSessionNo(db, fy) {
    const prefix = `PS/${fy}/`;
    const [rows] = await db.query("SELECT `sessionNo` FROM `physical_stock_sessions` WHERE `fy` = ? AND `sessionNo` LIKE ? ORDER BY CAST(SUBSTRING_INDEX(`sessionNo`,'/',-1) AS UNSIGNED) DESC LIMIT 1", [fy, `${prefix}%`]);
...
async function requirePhysicalStockAccess(req, res, next) {
    try {
        ...
        if (!hasPermission(user, "/physical-stock"))
            return res.status(403).json({ error: "Forbidden" });
        return next();
    }
    catch (error) {
...
app.post("/api/physical-stock/start-session", requireAuth, requirePhysicalStockAccess, async (req, res) => {
    ...
    try {
        ...
        await conn.query("INSERT INTO `physical_stock_sessions` (`id`, `sessionNo`, `sessionName`, `fy`, `status`, `startedAt`, `startedBy`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [session.id, session.sessionNo, session.sessionName, session.fy, session.status, session.startedAt, session.startedBy, session.updatedBy, session.updateTimestamp]);
        await conn.commit();
...
app.post("/api/physical-stock/restart-session", requireAuth, requirePhysicalStockAccess, async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        const updatedBy = String(user?.name || user?.email || user?.userId || "System").trim() || "System";
        await db.query("UPDATE `physical_stock_sessions` SET `status` = 'Open', `closedAt` = NULL, `closedBy` = NULL, `updatedBy` = ?, `updateTimestamp` = ? WHERE `id` = ?", [updatedBy, now, session.id]);
...
app.post("/api/physical-stock/close-session", requireAuth, requirePhysicalStockAccess, async (req, res) => {
    try {
        ...
        const openSession = await fetchOpenPhysicalStockSession(db);
        if (!openSession?.id)
            return res.status(404).json({ error: "No physical stock session is open." });
        ...
        await db.query("UPDATE `physical_stock_sessions` SET `status` = 'Closed', `closedAt` = ?, `closedBy` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE `id` = ?", [now, updatedBy, updatedBy, now, openSession.id]);
        return res.json(normalizeFetchedRow("physical_stock_sessions", {
            ...openSession,
            status: "Closed",
            closedAt: now,
        ...
        getAll: async (req, res) => {
          ...
          `);
                    ...
                    try {
                        ...
                        const [fallbackRows] = await db.query(`SELECT n.* FROM \`npd\` n ${fallbackWhereSql} ORDER BY n.npdId ASC LIMIT ? OFFSET ?`, [...fallbackParams, pageSize, (page - 1) * pageSize]);
                        const [fallbackCount] = await db.query(`SELECT COUNT(*) AS total FROM \`npd\` n ${fallbackWhereSql}`, fallbackParams);
                        result = { rows: fallbackRows, total: Number(fallbackCount[0]?.total || 0) };
                    }
          ...
          `, firmParams);
                ...
                const operationalRows = await excludeLegacyCorrugationInterFirmRows(db, tableName, rows);
                const processedRows = operationalRows.map((row) => {
                    const normalizedRow = normalizeFetchedRow(tableName, row);
                    if (tableName === "invoice_line_items") {
                        const resolvedItemName = String(row?.resolvedItemName ||
            ...
            try {
                ...
                if (tableName === "materials") {
                    ...
                    if (normalizedType === "Other") {
                        ...
                        [duplicateMaterialRows] = await db.query(`SELECT id, name FROM \`materials\`
               WHERE TRIM(COALESCE(erpCode, '')) = ? AND id <> ?
               LIMIT 1`, [rawErpCode, String(data.id || "")]);
                    }
                    else {
               ...
               WHERE TRIM(COALESCE(erpCode, '')) REGEXP '^[0-9]+$'
                 ...
                 AND id <> ?
               LIMIT 1`, [numericErpCode, String(data.id || "")]);
                    }
                    const duplicateMaterial = duplicateMaterialRows[0];
                ...
                if (tableName === "material_issues" || tableName === "material_returns") {
                    const lineTable = tableName === "material_issues" ? "material_issue_reel_lines" : "material_return_reel_lines";
                    const parentColumn = tableName === "material_issues" ? "materialIssueId" : "materialReturnId";
                    const recordId = String(data.id || "").trim();
                    if (recordId) {
                        const [reelRows] = await db.query(`SELECT id FROM \`${lineTable}\` WHERE \`${parentColumn}\` = ? LIMIT 1`, [recordId]);
                        if (reelRows[0]?.id) {
                            const unitOne = await getUnitOneFirm(db);
                            if (!unitOne?.id)
                                return res.status(400).json({ error: "Unit-I firm is not configured. Reel movements cannot be saved." });
                            data.firmId = String(unitOne.id);
                            delete data.firmName;
                        }
                    }
                }
                if (tableName === "material_issue_reel_lines" || tableName === "material_return_reel_lines") {
                ...
                if (tableName === "material_in_packing_slips") {
                    ...
                    const [duplicateReelRows] = await db.query(`SELECT id, ourReelNo FROM \`material_in_packing_slips\`
             WHERE LOWER(TRIM(ourReelNo)) = LOWER(TRIM(?))
               AND id <> ?
             LIMIT 1`, [normalizedReelNo, String(data.id || "")]);
                ...
                if (tableName === "settings") {
                    if (data.interFirmRatePercent !== undefined && data.interFirmRatePercent !== null) {
                        const rawRate = String(data.interFirmRatePercent).trim();
                        const rateValue = Number(rawRate);
                        if (!/^\d+(\.\d{1,2})?$/.test(rawRate) || !Number.isFinite(rateValue) || rateValue < 0 || rateValue > 100) {
                            return res.status(400).json({ error: "Inter-firm transaction rate must be between 0 and 100 with up to 2 decimal places." });
                        }
                        data.interFirmRatePercent = rateValue;
                    }
                    else {
                        data.interFirmRatePercent = 92;
                    }
                ...
                if (tableName === "machines") {
                    ...
                    const duplicateMachine = machineRows.find((machine) => normalizeMachineName(String(machine.name || "")).trim().toLowerCase() === normalizedName.toLowerCase());
                ...
                if (tableName === "users") {
                    ...
                    const [duplicateRows] = await db.query(`SELECT id, name,
                ${canonicalUserIdSql} as canonicalUserId
             FROM \`users\`
             WHERE LOWER(TRIM(${canonicalUserIdSql})) = LOWER(TRIM(?))
               AND id <> ?
             LIMIT 1`, [normalizedUserId, String(data.id || "")]);
                ...
                if (tableName === "npd") {
                    ...
                    if (normalizedErp) {
                        ...
                        const [existingNpdRows] = await db.query("SELECT id FROM `npd` WHERE COALESCE(NULLIF(TRIM(`erp`), ''), '') = ? AND id <> ? LIMIT 1", [normalizedErp, currentId]);
                ...
                if (tableName === "production_processing") {
                    ...
                    if (!String(data.machineId || "").trim())
                        missing.push("Machine");
                    if (!String(data.operatorId || "").trim())
                        missing.push("Operator");
                    if (!String(data.shift || "").trim())
                        missing.push("Shift");
                    ...
                    if (data.machineName === "Corrugation Liner" && completionStatus === "Full") {
                        ...
                        const [wastageProductionRows] = await db.query("SELECT `totalPaperWeight`, `qty`, `plannedQty` FROM `productions` WHERE `id` = ? LIMIT 1", [data.productionId]);
                        ...
                        const hasPairedBoxWastage = ["warpageBoxes", "delaminationBoxes", "misalignmentBoxes", "sheerCutterBoxes"]
                            .some((field) => Number(data[field] || 0) > 0);
               ...
               FROM (
                ...
                if (tableName === "reel_stock_taker_logs") {
                    ...
                    const [duplicateRows] = await db.query("SELECT `id` FROM `reel_stock_taker_logs` WHERE `sessionId` = ? AND LOWER(TRIM(`reelNo`)) = LOWER(TRIM(?)) AND `id` <> ? LIMIT 1", [openSession.id, reelNo, String(data.id || "")]);
                    ...
                    try {
                        ...
                        if (!alreadyApproved && (currentStatus === "Pending PH" || !currentStatus)) {
                            ...
                            if (itemId && Number.isFinite(orderRate) && punchDate) {
                                const [billingRows] = await db.query(`SELECT ili.rate
                     FROM \`invoice_line_items\` ili
                     JOIN \`invoices\` inv ON inv.id = ili.invoiceId
                     ...
                     LIMIT 1`, [itemId]);
                                ...
                                const ratesMatch = Number.isFinite(lastBillingRate1) && Math.abs(orderRate - lastBillingRate1) < 0.0001;
                                if (ratesMatch) {
                                    const [chartRows] = await db.query(`SELECT realizationRate
                     ...
                     LIMIT 1`, [punchDate]);
                                    ...
                                    if (Number.isFinite(realizationRate)) {
                                        const [rapcRows] = await db.query(`SELECT realizationPerKg
                       FROM \`productions\`
                ...
                if (['loading_slips', 'php_loading_slips', 'plate_loading_slips'].includes(tableName)) {
                    try {
                        if (!data.slipNo) {
                            data.slipNo = await generateLoadingSlipNo(db, tableName, data.date);
                        }
                    }
                    catch (err) {
                    ...
                    try {
                        ...
                        if (isWithoutJobIssueType(data.issueType)) {
                            ...
                            const [existingRows] = issueId
                                ...
                                : [[]];
                            const existingFirmId = String(existingRows?.[0]?.firmId || "").trim();
                   ...
                   LIMIT 1`, [duplicateFirmId, issueDate, issueId]);
                        ...
                        if (isWithoutJobIssueType(data.issueType)) {
                            if (!data.consumptionTransactionNo) {
                                data.consumptionTransactionNo = await generateSimpleTransactionNumber(db, "material_issues", "consumptionTransactionNo", "CON", issueDate);
                            }
                ...
                if (tableName === "material_in") {
                    if (String(data.mrrType || "").trim().toLowerCase() === "reel") {
                        const unitOne = await getUnitOneFirm(db);
                        if (!unitOne?.id)
                            return res.status(400).json({ error: "Unit-I firm is not configured. Reel receipts must belong to Unit-I." });
                        data.firmId = String(unitOne.id);
                        data.firmName = String(unitOne.firmName || "");
                    }
                ...
                if (["productions", "php_job_master", "plate_job_master"].includes(tableName)) {
                    try {
                        ...
                        return res.status(400).json({ error: `Could not generate unique production job number: ${err.message}` });
                    }
                }
                // New firm-owned documents always receive a server-authoritative serial.
                // Legacy records keep their existing numbers because they are detected by ID.
                const firmDocumentFields = {
                    orders: { field: "orderNo", code: "ORD", dateField: "date" },
                    indents: { field: "indentNo", code: "IND", dateField: "requisitionDate" },
                    gate_entries: { field: "gateEntryNo", code: "GE", dateField: "date" },
                    material_in: { field: "transactionNo", code: "MRR", dateField: "date" },
                    material_issues: { field: "issueNo", code: "MI", dateField: "date" },
                    material_returns: { field: "returnNo", code: "MR", dateField: "date" },
                    reel_transfers: { field: "transferNo", code: "RT", dateField: "date" },
                    productions: { field: "transactionNo", code: "JOB", dateField: "date" },
                    loading_slips: { field: "slipNo", code: "LS", dateField: "date" },
                    dispatch_plans: { field: "planNo", code: "DP", dateField: "date" },
                    gate_passes: { field: "gatePassNo", code: "GP", dateField: "date" },
                    invoices: { field: "invoiceNo", code: "INV", dateField: "date" },
                    credit_notes: { field: "creditNoteNo", code: "CN", dateField: "date" },
                    debit_notes: { field: "debitNoteNo", code: "DN", dateField: "date" },
                    consumptions: { field: "transactionNo", code: "CON", dateField: "date" },
                };
                const firmDocument = firmDocumentFields[tableName];
                ...
                if (["loading_slips", "php_loading_slips", "plate_loading_slips"].includes(tableName)) {
                    ...
                    for (const loadingSlipColumn of loadingSlipColumns) {
                        try {
                            ...
                            console.warn(`[DB] Could not ensure ${tableName}.${loadingSlipColumn.column}:`, error.message);
                        }
                    }
                    for (const tallyColumn of sharedTallyColumns) {
                        try {
                            ...
                            console.warn(`[DB] Could not ensure ${tableName}.${tallyColumn.column}:`, error.message);
                        }
                    }
                ...
                if (tableName === "gate_passes") {
                    ...
                    if (invoiceId) {
                        const currentId = String(data.id || "").trim();
                        const [existingRows] = await db.query("SELECT id, gatePassNo FROM `gate_passes` WHERE invoiceId = ? AND id <> ? LIMIT 1", [invoiceId, currentId]);
                ...
                if (tableName === "invoice_line_items") {
                    try {
                        ...
                        console.warn("[DB] Could not make invoice_line_items.loadingSlipId optional:", error.message);
                    }
                    const loadingSlipId = String(data.loadingSlipId || "").trim();
                    const isAutoInterFirmLine = String(data.interFirmFlow || "").toLowerCase() === "yes" ||
                        String(data.autoGenerated || "").toLowerCase() === "yes" ||
                        String(data.sourceFirmId || "").trim() !== "" ||
                        String(data.destinationFirmId || "").trim() !== "";
                    if (loadingSlipId && isAutoInterFirmLine) {
                        const [loadingSlipRows] = await db.query("SELECT id FROM `loading_slips` WHERE id = ? LIMIT 1", [loadingSlipId]);
                        if (loadingSlipRows.length === 0) {
                ...
                if (tableName === "orders") {
                    ...
                    try {
                        ...
                        if (id) {
                            const [existingRows] = await conn.query("SELECT status, approvedTimestamp, approvedEmail FROM `orders` WHERE id = ? LIMIT 1", [id]);
                            const existing = existingRows?.[0];
                        ...
                        const isNowApproved = hasWorkflowValue(data.approvedTimestamp) ||
                            hasWorkflowValue(data.approvedEmail) ||
                            approvalStatuses.has(nextStatus);
                        const shouldUpdateItemRate = !wasApproved &&
                            isNowApproved &&
                            itemId &&
                        ...
                        if (shouldUpdateItemRate) {
                            if (itemSource === "PHP") {
                                await conn.query("UPDATE `php_item_master` SET `rate` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE id = ?", [rateNumber, updatedBy, now, itemId]);
                            }
                            else if (itemSource === "PLATE") {
                                await conn.query("UPDATE `plate_item_master` SET `rate` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE id = ?", [rateNumber, updatedBy, now, itemId]);
                            }
                            else {
                                await conn.query("UPDATE `npd` SET `rate` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE id = ?", [rateNumber, updatedBy, now, itemId]);
                            }
                        }
                        if (itemSource === "FG" && itemId && Number.isFinite(rateNumber) && rateNumber > 0) {
                            await conn.query("UPDATE `npd` SET `orderRate` = ?, `updatedBy` = ?, `updateTimestamp` = ? WHERE id = ?", [rateNumber, updatedBy, now, itemId]);
                        }
                        await conn.commit();
                ...
                const isAtomicInterFirmProcessing = tableName === "production_processing" &&
                    ...
                    processingMachineName === "Printing";
                ...
                if (tableName === "materials" && materialOpening) {
                    ...
                    try {
                        await conn.beginTransaction();
                        await conn.query(query, values);
                        await conn.query(`INSERT INTO \`material_firm_openings\`
                (id, materialId, firmId, openingQty, openingRate, openingValue, updatedBy, updateTimestamp)
               VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?)
               ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate),
                 openingValue = VALUES(openingValue), updatedBy = VALUES(updatedBy), updateTimestamp = VALUES(updateTimestamp)`, [String(data.id || ""), materialOpening.firmId, materialOpening.qty, materialOpening.rate,
                ...
                if (tableName === "invoices" && String(data.interFirmFlow || "").trim().toLowerCase() === "yes") {
                    ...
                    if (pendingId) {
                        await db.query("UPDATE inter_firm_pending_invoices SET status = 'Invoiced', linkedInvoiceId = ?, updateTimestamp = ? WHERE id = ?", [String(data.id || ""), new Date().toISOString(), pendingId]);
                        const [pendingRows] = await db.query("SELECT linkedMrrId FROM inter_firm_pending_invoices WHERE id = ? LIMIT 1", [pendingId]);
                        const mrrId = String(pendingRows[0]?.linkedMrrId || data.linkedMrrId || "").trim();
                        if (mrrId) {
                            await db.query("UPDATE material_in SET invoiceNo = ?, invDate = ?, linkedInvoiceId = ? WHERE id = ?", [String(data.invoiceNo || ""), String(data.date || ""), String(data.id || ""), mrrId]);
                        }
                    }
                }
                if (tableName === "production_processing" && String(data.completionStatus || "").trim().toLowerCase() === "full") {
                    ...
                    // Printing is the only Production Processing stage that can create
                    // inter-firm documents. Corrugation Liner remains a Unit-I operation.
                    if (machineName === "Printing") {
                        await validateLnkiPrintingComponents(db, data);
            ...
            try {
                ...
                if (["productions", "production_processing"].includes(tableName)) {
                    ...
                    if (Number(linkedRows[0]?.cnt || 0) > 0) {
                        return res.status(409).json({ error: "Cannot delete source record while linked inter-firm records exist. Cancel the inter-firm transaction first." });
                    }
                }
...
function tallySyncSecretGuard(req, res, next) {
    ...
    console.info("[TALLY_SYNC_AUTH] Guard hit", JSON.stringify({
        method: req.method,
    ...
    if (!providedSecret || providedSecret !== TALLY_SYNC_SECRET) {
        console.warn("[TALLY_SYNC_AUTH] Unauthorized request", JSON.stringify({
            method: req.method,
            path: req.originalUrl,
            providedLength: providedSecret.length,
            expectedLength: TALLY_SYNC_SECRET.length,
...
async function fetchTallyInvoiceContext(db, invoiceId) {
    ...
    const itemLines = lineRows.map((row) => {
        ...
        const normalizedItemSource = itemSource === "PHP" || itemSource === "PLATE" || itemSource === "MATERIAL"
            ? itemSource
            : "FG";
        let itemName = "";
        let uom = "";
    ...
    const loadingSlipIds = Array.from(new Set(itemLines
    ...
    if (loadingSlipIds.length) {
      ...
      `, loadingSlipIds);
        for (const row of loadingSlipRows) {
            ...
            if (slipNo && !slipNos.includes(slipNo))
                slipNos.push(slipNo);
            if (truckNo && !truckNos.includes(truckNo))
                truckNos.push(truckNo);
            let slipLines = [];
            try {
    ...
    if (dispatchPlanIds.length) {
      ...
      `, dispatchPlanIds);
        for (const row of orderRows) {
            ...
            if (poNumber && !orderNos.includes(poNumber))
                orderNos.push(poNumber);
            if (orderDate && !orderDates.includes(orderDate))
...
app.get("/api/public/trucks", async (_req, res) => {
    ...
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    try {
        const [rows] = await db.query("SELECT id, truckNo, driverName, mobileNo, truckType, liveStatus, statusUpdatedAt FROM `trucks` WHERE COALESCE(NULLIF(TRIM(`truckType`), ''), 'External') = 'Internal' ORDER BY truckNo");
...
app.post("/api/public/truck-status", async (req, res) => {
    ...
    if (!truckId || !liveStatus)
        return res.status(400).json({ error: "Vehicle and valid status are required" });
    const db = await getPool();
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    try {
...
app.post("/api/truck-live-update", async (req, res) => {
    ...
    if (user.role === "TruckDriver")
        return res.status(403).json({ error: "Forbidden" });
    if (!hasPermission(user, "/reports/truck-status") && !hasPermission(user, "/truck/live-update"))
        return res.status(403).json({ error: "Forbidden" });
    const truckId = String(req.body?.truckId || "").trim();
...
app.get("/api/truck-driver/status", async (req, res) => {
    const user = await getRequestUser(req);
    if (!user || user.role !== "TruckDriver" || !user.truckId)
        return res.status(403).json({ error: "Forbidden" });
    const db = await getPool();
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    try {
        const [rows] = await db.query("SELECT id, truckNo, driverName, mobileNo, liveStatus, statusUpdatedAt, statusUpdatedBy FROM `trucks` WHERE id = ? LIMIT 1", [user.truckId]);
        const truck = rows[0];
        if (!truck)
...
app.get("/api/truck-status-logs", async (req, res) => {
    ...
    if (user.role === "TruckDriver")
        return res.status(403).json({ error: "Forbidden" });
    if (!hasPermission(user, "/reports/truck-status") && !hasPermission(user, "/truck/logs"))
        return res.status(403).json({ error: "Forbidden" });
...
app.post("/api/materials/opening-reels/bulk", async (req, res) => {
    ...
    if (inputRows.length === 0)
        return res.status(400).json({ error: "No opening-stock rows were provided." });
    const db = await getPool();
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    const conn = await db.getConnection();
    ...
    try {
        ...
        if (!unitOne?.id)
            fail("Unit-I firm is not configured. Reel opening stock must belong to Unit-I.");
        ...
        const existingReelByKey = new Map();
        packingSlipRows.forEach((row) => {
            const key = normalizeKey(row.ourReelNo);
            if (key && !existingReelByKey.has(key))
                existingReelByKey.set(key, row);
        });
        let reelGroup = groupRows.find((row) => normalizeKey(row.name) === "reel");
        if (!reelGroup) {
            reelGroup = { id: crypto.randomUUID(), name: "Reel" };
            await conn.query("INSERT INTO `material_groups` (`id`, `name`, `updatedBy`, `updateTimestamp`) VALUES (?, 'Reel', ?, ?)", [reelGroup.id, user.name || user.userId || "System", new Date().toISOString()]);
        }
        ...
        inputRows.forEach((raw, index) => {
            ...
            if (!Number.isFinite(weightKg) || weightKg <= 0)
                fail(`Row ${rowNumber}: Opening Stock KG must be greater than 0.`);
            if (!Number.isFinite(openingRate) || openingRate < 0)
                fail(`Row ${rowNumber}: Opening Rate must be zero or greater.`);
            ...
            if (priorSignature && priorSignature !== signature)
                fail(`Row ${rowNumber}: Repeated ERP Code ${erpCode} must use the same Size, GSM, BF, Color, Remarks, and Active values.`);
        ...
        for (const supplierName of Array.from(new Set(normalizedRows.map((row) => normalizeKey(row.supplierName))))) {
            ...
            await conn.query("INSERT INTO `suppliers` (`id`, `name`, `firmId`, `gstSupplyType`, `active`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, 'INTRA_STATE', 'Yes', ?, ?)", [supplier.id, supplier.name, unitOne.id, actor, timestamp]);
        ...
        for (const [erpCode, rowsForMaterial] of rowsByErp.entries()) {
            ...
            if (existingMaterial) {
                await conn.query(`UPDATE \`materials\` SET type = 'Reel', erpCode = ?, name = ?, uom = 'KGS', materialGroupId = ?, color = ?, size = ?, gsm = ?, bf = ?, remarks = ?, active = ?, updatedBy = ?, updateTimestamp = ? WHERE id = ?`, [erpCode, displayName, reelGroup.id, sample.color, sample.size, sample.gsm, sample.bf, sample.remarks || null, sample.active, actor, timestamp, materialId]);
                updatedMaterials += 1;
            }
            else {
        ...
        for (const materialId of Array.from(affectedMaterialIds).filter(Boolean)) {
         ...
         FROM \`material_in_packing_slips\`
         WHERE materialId = ? AND firmId = ? AND UPPER(TRIM(materialInId)) = 'OPENING'`, [materialId, unitOne.id]);
            ...
            await conn.query(`INSERT INTO \`material_firm_openings\` (id, materialId, firmId, openingQty, openingRate, openingValue, updatedBy, updateTimestamp)
         VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate), openingValue = VALUES(openingValue), updatedBy = VALUES(updatedBy), updateTimestamp = VALUES(updateTimestamp)`, [materialId, unitOne.id, openingQty, openingRate, openingValue, actor, timestamp]);
        }
        await conn.commit();
...
app.post("/api/materials/other-materials/bulk", async (req, res) => {
    ...
    const inputRows = Array.isArray(req.body?.rows) ? req.body.rows : [];
    if (inputRows.length === 0)
        return res.status(400).json({ error: "No Other material rows were provided." });
    const db = await getPool();
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    const conn = await db.getConnection();
    ...
    try {
        ...
        firmRows.forEach((row) => {
            [row.firmName, row.shortName].forEach((name) => {
                ...
                if (key && !firmByName.has(key))
                    firmByName.set(key, row);
            });
        });
        const materialByErp = new Map(materialRows.filter((row) => String(row.erpCode || "").trim()).map((row) => [String(row.erpCode).trim(), row]));
        const materialByName = new Map(materialRows.filter((row) => String(row.name || "").trim()).map((row) => [normalizeKey(row.name), row]));
        const groupByName = new Map(groupRows.map((row) => [normalizeKey(row.name), row]));
        const normalizedRows = [];
        const masterSignatureByErp = new Map();
        const masterSignatureByName = new Map();
        const openingRowByKey = new Map();
        ...
        inputRows.forEach((raw, index) => {
            ...
            const unsupportedField = Object.entries(raw || {}).find(([field, value]) => !allowedFields.has(field) && String(value ?? "").trim() !== "");
            if (unsupportedField)
                fail(`Row ${rowNumber}: ${unsupportedField[0]} is not supported in the Other Material upload.`);
            ...
            if (!firmName)
                fail(`Row ${rowNumber}: Firm Name or Short Name is required.`);
            const firm = firmByName.get(normalizeKey(firmName));
            if (!firm)
                fail(`Row ${rowNumber}: Firm Name or Short Name ${firmName} was not found.`);
            if (!/^\d{8}$/.test(rawErp))
                fail(`Row ${rowNumber}: ERP Code must be exactly 8 digits for Other materials.`);
            if (!itemName)
                fail(`Row ${rowNumber}: Item Name is required.`);
            if (!itemGroup)
                fail(`Row ${rowNumber}: Item Group is required.`);
            if (!unit)
                fail(`Row ${rowNumber}: Unit is required.`);
            if (!rawOpeningStock || !Number.isFinite(openingStock) || openingStock < 0)
                fail(`Row ${rowNumber}: Opening Stock must be zero or greater.`);
            if (!rawOpeningRate || !Number.isFinite(openingRate) || openingRate < 0)
                fail(`Row ${rowNumber}: Opening Rate must be zero or greater.`);
        ...
        const materialIdByErp = new Map();
        for (const row of normalizedRows) {
            if (materialIdByErp.has(row.erpCode))
                continue;
            const byErp = materialByErp.get(row.erpCode);
            const byName = materialByName.get(normalizeKey(row.itemName));
            if (byErp && byName && String(byErp.id) !== String(byName.id))
                fail(`Row ${row.rowNumber}: ERP Code ${row.erpCode} and Item Name ${row.itemName} belong to different existing materials.`, 409);
            const existingMaterial = byErp || byName;
            if (existingMaterial && String(existingMaterial.type || "").trim() !== "Other")
                fail(`Row ${row.rowNumber}: ERP Code or Item Name already belongs to a non-Other material.`, 409);
            ...
            const materialId = String(existingMaterial?.id || crypto.randomUUID());
            if (existingMaterial) {
                await conn.query(`UPDATE \`materials\` SET type = 'Other', erpCode = ?, name = ?, uom = ?, materialGroupId = ?, color = NULL, size = NULL, gsm = NULL, bf = NULL, updatedBy = ?, updateTimestamp = ? WHERE id = ?`, [row.erpCode, row.itemName, row.unit, group.id, actor, timestamp, materialId]);
                updatedMaterials += 1;
            }
            else {
                await conn.query(`INSERT INTO \`materials\` (id, type, erpCode, name, uom, materialGroupId, active, updatedBy, updateTimestamp) VALUES (?, 'Other', ?, ?, ?, ?, 'Yes', ?, ?)`, [materialId, row.erpCode, row.itemName, row.unit, group.id, actor, timestamp]);
                createdMaterials += 1;
            }
        ...
        for (const row of openingRowByKey.values()) {
            ...
            await conn.query(`INSERT INTO \`material_firm_openings\` (id, materialId, firmId, openingQty, openingRate, openingValue, updatedBy, updateTimestamp)
         VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate), openingValue = VALUES(openingValue), updatedBy = VALUES(updatedBy), updateTimestamp = VALUES(updateTimestamp)`, [materialId, row.firmId, row.openingStock, row.openingRate, openingValue, actor, timestamp]);
        ...
        return res.status(statusCode).json({ error: error.message || "Other material upload failed." });
    }
    finally {
        conn.release();
    }
...
app.get("/api/material-firm-openings", requireAuth, async (req, res) => {
    try {
        ...
        if (requestedFirmId === "all") {
            ...
            return res.json(rows.map((row) => normalizeFetchedRow("material_firm_openings", row)));
        }
        if (!requestedFirmId)
...
app.post("/api/material-firm-openings", requireAuth, async (req, res) => {
    try {
        ...
        // firm ID.
        const requestedFirmId = getRequestFirmId(req);
        let firmId = String(req.body?.firmId || (requestedFirmId === "all" ? "" : requestedFirmId) || "").trim();
        ...
        const [materialRows] = await db.query("SELECT id, type FROM `materials` WHERE id = ? LIMIT 1", [materialId]);
        ...
        const timestamp = new Date().toISOString();
        await db.query(`INSERT INTO \`material_firm_openings\` (id, materialId, firmId, openingQty, openingRate, openingValue, updatedBy, updateTimestamp)
       VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE openingQty = VALUES(openingQty), openingRate = VALUES(openingRate), openingValue = VALUES(openingValue), updatedBy = VALUES(updatedBy), updateTimestamp = VALUES(updateTimestamp)`, [materialId, firmId, qty, rate, value, actor, timestamp]);
        return res.json({ success: true });
    }
    catch (error) {
        return res.status(Number(error?.statusCode || 500)).json({ error: error.message });
...
app.post("/api/settings/clear-core-job-data", async (req, res) => {
    ...
    if (!user)
        return res.status(401).json({ error: "Unauthorized" });
    if (user.role !== "Admin")
        return res.status(403).json({ error: "Only administrators can clear core job data." });
    ...
    try {
        ...
        return res.status(500).json({ error: error.message || "Failed to clear core job data.", backupTables });
    }
    finally {
        conn.release();
    }
...
app.get("/api/legacy-items", async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(401).json({ error: "Unauthorized" });
        if (!hasPermission(user, "/masters/items"))
            return res.status(403).json({ error: "Forbidden" });
        const db = await getPool();
        if (!db)
            return res.status(500).json({ error: "DB connection not available" });
...
app.get("/api/purchase-orders/pending-indent-lines", requireAuth, async (req, res) => {
    ...
    `, [requestedFirmId, requestedFirmId]);
        ...
        const lastPoInfoByMaterial = new Map();
        ...
        `, materialIds);
            rateRows.forEach((r) => {
                ...
                if (!mid || lastPoInfoByMaterial.has(mid))
                    return;
    ...
    try {
      ...
      `, lineIds);
        ...
        normalizedLines.forEach((l) => {
            if (!groups.has(l.supplierId))
                groups.set(l.supplierId, []);
            groups.get(l.supplierId).push(l);
        });
        const created = [];
        const indentLinesToUpdate = new Map();
        for (const [supplierId, groupLines] of groups.entries()) {
            const poNo = await generateFirmDocumentNo(conn, purchaseFirmId, "PO", dateStr);
            ...
            const indentIdsForOrder = new Set();
            for (const l of groupLines) {
                ...
                if (row?.indentId)
                    indentIdsForOrder.add(String(row.indentId));
            }
            ...
            for (const line of poLinesToInsert) {
                await conn.query("INSERT INTO `purchase_order_lines` (id, purchaseOrderId, indentLineId, materialId, erpCode, uom, qty, rate, amount, gstRate, cgst, sgst, igst, lineTotal, targetDeliveryDate, updatedBy, updateTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
                    line.id,
        ...
        for (const [indentLineId, addQty] of indentLinesToUpdate.entries()) {
            ...
            if (row?.indentId)
                indentIdsToCheck.add(String(row.indentId));
        }
      ...
      `, [materialId, uom]);
            for (const source of sourceRows) {
                ...
                if (allocate > 0) {
                    ...
                    if (source.indentId)
                        indentIdsForOrder.add(String(source.indentId));
                }
            }
        }
        if (poLines.length === 0)
            throw new Error("No quantities allocated to indent lines.");
        ...
        for (const line of poLines) {
            await conn.query("INSERT INTO `purchase_order_lines` (id, purchaseOrderId, indentLineId, materialId, erpCode, uom, qty, rate, amount, gstRate, cgst, sgst, igst, lineTotal, targetDeliveryDate, updatedBy, updateTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [line.id, line.purchaseOrderId, line.indentLineId, line.materialId, line.erpCode, line.uom, line.qty, line.rate, line.amount, line.gstRate, line.cgst, line.sgst, line.igst, line.lineTotal, line.targetDeliveryDate, auditActor, auditTimestamp]);
        }
        const indentIdsToCheck = new Set();
        for (const [id, addQty] of indentLinesToUpdate.entries()) {
            ...
            if (ilRows.length)
                indentIdsToCheck.add(ilRows[0].indentId);
        }
        for (const indentId of indentIdsToCheck) {
...
app.post("/api/get-pending-job-closure", async (req, res) => {
    ...
    if (!user)
        return res.status(401).json({ error: "Unauthorized" });
    if (!hasPermission(user, "/production"))
        return res.status(403).json({ error: "Forbidden" });
    ...
    try {
        ...
        const [processingRows] = productionIds.length
            ? await db.query(`SELECT productionId, machineId, machineName, shift, qty, operatorId, date, completionStatus FROM \`production_processing\` WHERE productionId IN (${productionIds
                .map(() => "?")
                .join(",")})`, productionIds)
            : [[]];
        ...
        const rows = productions
            ...
            requiredSteps.forEach((stepMachineName) => {
                ...
                const completedRecord = stepRecords.find((r) => {
                    ...
                    if (!Number.isFinite(qtyValue) || qtyValue <= 0)
                        return false;
                    if (!String(r.machineId || "").trim())
                        return false;
                    if (!String(r.operatorId || "").trim())
                        return false;
                    if (!String(r.shift || "").trim())
                        return false;
                    if (!String(r.date || "").trim())
                        return false;
                    if (String(r.completionStatus || "").trim() && String(r.completionStatus || "").trim() !== "Full")
                        return false;
                    return true;
                });
                if (!completedRecord) {
            ...
            if (missingSteps.length)
                blockingReasons.push(`Missing processing steps: ${missingSteps.map((s) => s.machineName).join(", ")}`);
            if (missingFields.length)
                blockingReasons.push(`Incomplete processing entries: ${missingFields.map((s) => s.machineName).join(", ")}`);
            ...
            const groupMachineId = groupMachineName && groupMachineName !== "Multiple/Various" && groupMachineName !== "Unassigned"
...
app.post("/api/settings/global-item-rename", async (req, res) => {
    ...
    if (!config)
        return res.status(400).json({ error: "Invalid item source." });
    if (!itemId)
        return res.status(400).json({ error: "Item is required." });
    if (!newName)
        return res.status(400).json({ error: "New item name is required." });
    const db = await getPool();
    if (!db)
        return res.status(500).json({ error: "DB connection not available" });
    try {
        ...
        const [rows] = await db.query(`SELECT id, \`${config.nameColumn}\` AS currentName FROM \`${config.tableName}\` WHERE id = ? LIMIT 1`, [itemId]);
        const row = rows[0];
        if (!row)
...
const GLOBAL_ITEM_TRANSFER_MODULES = new Set([
    "orders",
    "plans",
    "dispatch",
    "loading",
...
function normalizeGlobalItemMaintenanceSource(value) {
    ...
    return GLOBAL_ITEM_RENAME_SOURCE_CONFIG[normalized]
        ? normalized
        : null;
}
...
async function getGlobalItemTransferIdentity(db, database, source, itemId) {
    ...
    if (!columns.has("id") || !columns.has(config.nameColumn))
        return null;
    ...
    const erpExpr = erpColumns.length
        ...
        : "''";
    const [rows] = await db.query(`SELECT \`id\`, \`${config.nameColumn}\` AS itemName, ${erpExpr} AS erp FROM \`${config.tableName}\` WHERE \`id\` = ? LIMIT 1`, [itemId]);
    const row = rows[0];
    if (!row)
        return null;
    return {
...
function transferLinkedItemInArray(rows, source, fromItemId, toItem) {
    ...
    const nextRows = rows.map((row) => {
        ...
        if (rowItemId !== fromItemId || rowSource !== source)
            return row;
        changed = true;
        lineCount += 1;
        return {
...
async function collectLoadingTransferRows(db, database, source, fromItemId, toItem, actor, now) {
    ...
    if (source === "PHP")
        loadingTables.push("php_loading_slips");
    if (source === "PLATE")
        loadingTables.push("plate_loading_slips");
    let records = 0;
    let lines = 0;
    const tableCounts = {};
    const detailRows = [];
    for (const tableName of loadingTables) {
        ...
        if (!columns.has("id") || !columns.has("lines"))
            continue;
        ...
        for (const row of rows) {
            ...
            if (lineTransfer.changed) {
                ...
                originalLines.forEach((line) => {
                    ...
                    if (String(line?.itemId || "").trim() !== fromItemId || lineSource !== source)
                        return;
                    detailRows.push({
            ...
            for (const detailColumn of optionalJsonColumns) {
                ...
                if (detailTransfer.changed) {
                    ...
                    originalDetails.forEach((detail) => {
                        ...
                        if (String(detail?.itemId || "").trim() !== fromItemId || detailSource !== source)
                            return;
                        detailRows.push({
...
async function countGlobalItemTransfer(db, database, source, fromItemId, modules) {
    ...
    if (modules.includes("orders")) {
        const [rows] = await db.query(`SELECT o.id, o.orderNo, o.orderDate, o.qty, o.status
       FROM \`orders\` o
       WHERE ${orderCondition.sql}
       ORDER BY o.orderDate DESC, o.orderNo DESC`, orderCondition.buildParams(fromItemId));
    ...
    if (modules.includes("plans")) {
        const [scheduleRows] = await db.query(`SELECT os.id, os.scheduleNo, os.scheduledDate, os.qty, os.producedQty, os.canceledQty, o.orderNo
       FROM \`orders_schedule\` os
       JOIN \`orders\` o ON o.id = os.orderId
       WHERE ${orderCondition.sql}
       ORDER BY os.scheduledDate DESC, os.scheduleNo DESC`, orderCondition.buildParams(fromItemId));
        const [productionRows] = await db.query(`SELECT p.id, p.transactionNo, p.jobCardNo, p.date, p.qty, p.status, os.scheduleNo, o.orderNo
       FROM \`productions\` p
       LEFT JOIN \`orders_schedule\` os ON os.id = p.scheduleId
       LEFT JOIN \`orders\` o ON o.id = os.orderId
       WHERE ${productionCondition.sql}
       ORDER BY p.date DESC, p.transactionNo DESC`, productionCondition.buildParams(fromItemId));
    ...
    if (modules.includes("dispatch")) {
        const [rows] = await db.query(`SELECT DISTINCT dp.id, dp.planNo, dp.date, dp.plannedQty, dp.loadedQty, dp.status, o.orderNo, p.transactionNo, p.jobCardNo
       FROM \`dispatch_plans\` dp
       LEFT JOIN \`orders\` o ON o.id = dp.orderId
       LEFT JOIN \`productions\` p ON p.id = dp.productionId
       WHERE ${orderCondition.sql} OR ${productionCondition.sql}
       ORDER BY dp.date DESC, dp.planNo DESC`, [...orderCondition.buildParams(fromItemId), ...productionCondition.buildParams(fromItemId)]);
    ...
    if (modules.includes("billing")) {
        const [rows] = await db.query(`SELECT ili.id, ili.qty, ili.rate, ili.amount, inv.invoiceNo, inv.date AS invoiceDate, ls.slipNo
       FROM \`invoice_line_items\` ili
       ...
       ORDER BY inv.date DESC, inv.invoiceNo DESC`, invoiceCondition.buildParams(fromItemId));
...
async function applyGlobalItemTransfer(db, database, user, source, fromItemId, toItem, modules) {
    ...
    const [preOrderRows] = await db.query(`SELECT \`id\` FROM \`orders\` WHERE ${orderCondition.sql}`, orderCondition.buildParams(fromItemId));
    ...
    if (modules.includes("plans") && preMatchedOrderIds.length > 0) {
        ...
        const [scheduleRows] = await db.query(`SELECT \`id\` FROM \`orders_schedule\` WHERE \`orderId\` IN (${placeholders})`, preMatchedOrderIds);
        preMatchedScheduleIds = scheduleRows.map((row) => String(row.id || "")).filter(Boolean);
    }
    let preMatchedDispatchIds = [];
    if (modules.includes("dispatch")) {
        const [dispatchRows] = await db.query(`SELECT DISTINCT dp.id
       FROM \`dispatch_plans\` dp
       LEFT JOIN \`orders\` o ON o.id = dp.orderId
       LEFT JOIN \`productions\` p ON p.id = dp.productionId
       WHERE ${orderAliasCondition.sql} OR ${productionAliasCondition.sql}`, [...orderAliasCondition.buildParams(fromItemId), ...productionAliasCondition.buildParams(fromItemId)]);
    ...
    if (modules.includes("plans")) {
        ...
        if (scheduleColumns.has("id") &&
            ...
            const [result] = await db.query(`UPDATE \`orders_schedule\` SET ${updateParts.join(", ")} WHERE \`id\` IN (${placeholders})`, params);
    ...
    if (modules.includes("dispatch") &&
        ...
        const [result] = await db.query(`UPDATE \`dispatch_plans\` SET ${updateParts.join(", ")} WHERE \`id\` IN (${placeholders})`, params);
...
app.post("/api/settings/item-transfer/preview", async (req, res) => {
    try {
        ...
        if (!validated)
            return;
        const { counts, details } = await countGlobalItemTransfer(validated.db, validated.database, validated.source, validated.fromItem.id, validated.modules);
        return res.json({
            success: true,
...
app.post("/api/settings/item-transfer/apply", async (req, res) => {
    ...
    try {
        ...
        const { counts, details } = await applyGlobalItemTransfer(connection, validated.database, validated.user, validated.source, validated.fromItem.id, validated.toItem, validated.modules);
...
app.post("/api/reel-transfers/execute", async (req, res) => {
    const user = await getRequestUser(req);
    if (!user)
        return res.status(401).json({ error: "Unauthorized" });
    if (!hasPermission(user, "/material-movement/reel-transfer"))
        return res.status(403).json({ error: "Forbidden" });
    ...
    try {
        ...
        await conn.beginTransaction();
        const [productionRows] = await conn.query("SELECT * FROM `productions` WHERE `id` IN (?, ?) FOR UPDATE", [sourceProductionId, targetProductionId]);
        const productions = productionRows;
        const source = productions.find((row) => String(row.id) === sourceProductionId);
        const target = productions.find((row) => String(row.id) === targetProductionId);
        if (!source || !target)
            throw new Error("Source or target job was not found.");
        if (String(source.status || "") === "Cancelled" || source.cancelTimestamp)
            throw new Error("The source job is cancelled.");
        ...
        transferStage = "reading corrugation records";
        const [processingRows] = await conn.query("SELECT `productionId`, `machineName`, `qty`, `completionStatus`, `date`, `updateTimestamp` FROM `production_processing` WHERE `productionId` IN (?, ?)", [sourceProductionId, targetProductionId]);
        const processing = processingRows;
        const corrugationRows = (productionId) => processing.filter((row) => String(row.productionId) === productionId &&
            String(row.machineName || "").trim().toLowerCase().replace(/\s+/g, " ") === "corrugation liner");
        ...
        const sourceFull = sourceCorrugation
            .filter((row) => String(row.completionStatus || "").trim().toLowerCase() === "full")
            .map((row) => ({ ...row, time: processingTime(row) }))
            .filter((row) => Number.isFinite(row.time) && row.time > 0)
            .sort((a, b) => a.time - b.time)[0];
        if (!sourceFull)
            throw new Error("Corrugation Liner must be marked Full for the source job.");
        if (corrugationRows(targetProductionId).length > 0) {
            throw new Error("Corrugation Liner has already started for the target job.");
        }
        transferStage = "reading transfer settings";
        ...
        if (!Number.isFinite(windowHours) || windowHours <= 0 || Date.now() > sourceFull.time + windowHours * 60 * 60 * 1000) {
            throw new Error(`The ${windowHours || 12}-hour reel transfer window has expired.`);
        }
        transferStage = "checking target reel issues";
        const [targetIssueRows] = await conn.query("SELECT COUNT(*) AS `count` FROM `material_issue_reel_lines` WHERE `productionId` = ? OR LOWER(TRIM(`jobNo`)) = LOWER(TRIM(?))", [targetProductionId, targetJobNo]);
        if (Number(targetIssueRows[0]?.count || 0) > 0)
            throw new Error("The target job already has a reel issue.");
        transferStage = "locking source reel issues";
        const [issueRows] = await conn.query(`SELECT ril.*, COALESCE(mil.rate, mil.lastPurchaseRate, mil.openingRate, 0) AS rate,
              COALESCE(mil.openingRate, 0) AS openingRate, COALESCE(mil.lastPurchaseRate, 0) AS lastPurchaseRate
       FROM \`material_issue_reel_lines\` ril
       LEFT JOIN \`material_issue_lines\` mil ON mil.\`id\` = ril.\`materialIssueLineId\`
       WHERE ril.\`productionId\` = ? OR LOWER(TRIM(ril.\`jobNo\`)) = LOWER(TRIM(?)) FOR UPDATE`, [sourceProductionId, sourceJobNo]);
        transferStage = "locking source reel returns";
        const [returnRows] = await conn.query("SELECT * FROM `material_return_reel_lines` WHERE `productionId` = ? OR LOWER(TRIM(`jobNo`)) = LOWER(TRIM(?)) FOR UPDATE", [sourceProductionId, sourceJobNo]);
        const issues = issueRows;
        const returns = returnRows;
        const normalizeReelIdentity = (value) => String(value || "").trim().toLowerCase();
        const positiveReturns = returns.filter((row) => Number(row.weightKg || 0) > 0.004);
        const returnedSlipIds = new Set(positiveReturns.map((row) => String(row.packingSlipId || "").trim()).filter(Boolean));
        const returnedReelNos = new Set(positiveReturns.map((row) => normalizeReelIdentity(row.ourReelNo)).filter(Boolean));
        const isReturnedReel = (row) => returnedSlipIds.has(String(row.packingSlipId || "").trim()) ||
            returnedReelNos.has(normalizeReelIdentity(row.ourReelNo));
        const balanceBySlip = new Map();
        const originalIssuedBySlip = new Map();
        issues.forEach((row) => {
            if (isReturnedReel(row))
                return;
        ...
        if (planQty <= 0)
            throw new Error("Source job plan quantity is required for notional transfer calculation.");
        if (requiredKg <= 0)
            throw new Error("Source job weight calculation is unavailable for notional transfer.");
        ...
        if (notionalLeftKg <= 0.004)
            throw new Error("No unused reel weight remains after Corrugation.");
        ...
        if (outstandingOriginalIssuedKg <= 0)
            throw new Error("No valid original issued weight is available for transfer.");
        ...
        const selected = selectedCandidates.map((row) => {
            ...
            if (weightKg <= 0)
                throw new Error(`No notional transfer weight remains for reel ${row.issue.ourReelNo || row.issue.packingSlipId}.`);
            return { issue: row.issue, weightKg, rate: Number(row.issue.rate || 0), amount: Number((weightKg * Number(row.issue.rate || 0)).toFixed(2)) };
        });
        transferStage = "checking duplicate transfers";
        const [priorTransferReturns] = await conn.query(`SELECT \`packingSlipId\` FROM \`material_return_reel_lines\`
       WHERE \`productionId\` = ? AND \`jobTransfer\` = 'Yes' AND \`packingSlipId\` IN (${packingSlipIds.map(() => "?").join(",")})`, [sourceProductionId, ...packingSlipIds]);
        if (priorTransferReturns.length)
            throw new Error("A selected reel already has a job-transfer return against the source job.");
        const timestamp = new Date().toISOString();
        const transferId = crypto.randomUUID();
        const returnId = crypto.randomUUID();
        const issueId = crypto.randomUUID();
        const transferNo = await generateSimpleTransactionNumber(conn, "reel_transfers", "transferNo", "RT", transferDate);
        const returnNo = await generateSimpleTransactionNumber(conn, "material_returns", "returnNo", "MR", transferDate);
        const issueNo = await generateSimpleTransactionNumber(conn, "material_issues", "issueNo", "MIS", transferDate);
        const totalWeightKg = Number(selected.reduce((sum, row) => sum + row.weightKg, 0).toFixed(2));
        const totalAmount = Number(selected.reduce((sum, row) => sum + row.amount, 0).toFixed(2));
        const actor = String(user.name || user.email || "System User");
        const note = [`Reel transfer ${transferNo}`, remarks].filter(Boolean).join(" - ");
        const unitOne = await getUnitOneFirm(conn);
        const unitOneFirmId = String(unitOne?.id || "").trim();
        if (!unitOneFirmId)
            throw new Error("Unit I firm is not configured.");
        transferStage = "creating Unit-I reel return";
        ...
        transferStage = "creating Unit-I reel issue";
        ...
        transferStage = "creating transfer header";
        ...
        selected.forEach((row) => byMaterial.set(String(row.issue.materialId), [...(byMaterial.get(String(row.issue.materialId)) || []), row]));
        for (const [materialId, rows] of byMaterial) {
            const qty = Number(rows.reduce((sum, row) => sum + row.weightKg, 0).toFixed(2));
            const amount = Number(rows.reduce((sum, row) => sum + row.amount, 0).toFixed(2));
            const rate = qty > 0 ? Number((amount / qty).toFixed(2)) : 0;
            const returnLineId = crypto.randomUUID();
            const issueLineId = crypto.randomUUID();
            transferStage = "creating transfer material lines";
            await conn.query("INSERT INTO `material_return_lines` (`id`, `materialReturnId`, `materialId`, `qty`, `uom`, `rate`, `amount`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, ?, 'KG', ?, ?, ?, ?)", [returnLineId, returnId, materialId, qty, rate, amount, actor, timestamp]);
            await conn.query("INSERT INTO `material_issue_lines` (`id`, `materialIssueId`, `materialId`, `qty`, `uom`, `rate`, `amount`, `updatedBy`, `updateTimestamp`) VALUES (?, ?, ?, ?, 'KG', ?, ?, ?, ?)", [issueLineId, issueId, materialId, qty, rate, amount, actor, timestamp]);
            for (const row of rows) {
                const returnReelLineId = crypto.randomUUID();
                const issueReelLineId = crypto.randomUUID();
                const transferLineId = crypto.randomUUID();
                transferStage = "creating reel return line";
                ...
                transferStage = "creating reel issue line";
                ...
                transferStage = "creating reel transfer line";
        ...
        for (const production of [source, target]) {
            const productionId = String(production.id);
            const jobNo = String(production.transactionNo || "");
            transferStage = "updating job reel usage";
            const [usageRows] = await conn.query(`SELECT
          COALESCE((SELECT SUM(\`weightKg\`) FROM \`material_issue_reel_lines\` WHERE \`productionId\` = ? OR LOWER(TRIM(\`jobNo\`)) = LOWER(TRIM(?))), 0) -
          COALESCE((SELECT SUM(\`weightKg\`) FROM \`material_return_reel_lines\` WHERE \`productionId\` = ? OR LOWER(TRIM(\`jobNo\`)) = LOWER(TRIM(?))), 0) AS \`usage\``, [productionId, jobNo, productionId, jobNo]);
        ...
        transferStage = "recording source transfer details";
        ...
        await conn.rollback();
        console.error("[REEL_TRANSFER] failed:", error);
        const message = error instanceof Error ? error.message : "Failed to transfer reels.";
        return res.status(400).json({ error: `Reel transfer failed while ${transferStage}: ${message}` });
    }
    finally {
        conn.release();
    }
