...
async function getFirmDocumentPrefix(db, firmId) {
    const normalizedFirmId = String(firmId || "").trim();
    if (!normalizedFirmId)
        return "LNGRP";
...
async function automateInterFirmProduction(db, sourceId, sourceType = "Production", providedConnection) {
    ...
    try {
        ...
        const eligibleProcessingRecord = sourceType === "Production Processing" &&
            ["Corrugation Liner", "Printing"].includes(normalizeMachineName(String(sourceRecord?.machineName || ""))) &&
            String(sourceRecord?.completionStatus || "").trim().toLowerCase() === "full";
        if (!eligibleProcessingRecord) {
            await skip();
            return;
        }
        ...
        if (sourceType === "Production Processing" && normalizedMachine === "Corrugation Liner") {
            ...
            // Corrugation is physically performed and owned by Unit-I. A non-Unit-I
            // order hands the output to Unit-II for the downstream production stages.
            sourceFirmId = String(unit1.id);
            if (orderFirmId === sourceFirmId) {
                await skip();
                return;
            }
            destinationFirmId = String(unit2.id);
        }
    ...
    for (let i = 0; i < retries; i++) {
      ...
      `);
            ...
            const migrations = [
                ...
                { table: "productions", column: "idToOd2", type: "DECIMAL(15,2)" },
