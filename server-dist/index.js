...
app.post("/api/reel-transfers/execute", async (req, res) => {
    ...
    try {
       ...
       WHERE ril.productionId = ? OR LOWER(TRIM(ril.jobNo)) = LOWER(TRIM(?)) FOR UPDATE`, [sourceProductionId, sourceJobNo]);
        ...
        const [unitTwoRows] = await conn.query("SELECT id FROM firms WHERE active <> 'No' AND (LOWER(firmName) LIKE '%unit ii%' OR LOWER(firmName) LIKE '%unit-ii%' OR LOWER(firmName) LIKE '%unit 2%') ORDER BY firmName LIMIT 1");
        const unitTwoFirmId = String(unitTwoRows[0]?.id || "").trim();
        if (!unitTwoFirmId)
