...
function processBatchSync_(config, spreadsheet, sheet, allRowsToSync, allRowIndicesToUpdate, hostingerSyncIndex) {
  ...
  for (let i = 0; i < allRowsToSync.length; i += batchSize) {
    ...
    try {
      const targetResult = postSyncPayloadToTargets_(config, {
        spreadsheetId: config.spreadsheetId,
        ...
        rows: rowsToSync,
      }, `Full sync batch for ${config.tabName}`);
      hadPartialFailure = hadPartialFailure || targetResult.failures.length > 0;

      const parsedResponse = targetResult.primary;
      logItemMasterSync_(config, 'Received full-sync response', {
        ...
        targetResults: targetResult.results,
      });
  ...
  const finalizeResult = postSyncPayloadToTargets_(config, {
    spreadsheetId: config.spreadsheetId,
    spreadsheetName: spreadsheet.getName(),
    tabName: sheet.getName(),
  ...
  }, `Full sync finalize for ${config.tabName}`);
  hadPartialFailure = hadPartialFailure || finalizeResult.failures.length > 0;

  const parsedFinalizeResponse = finalizeResult.primary;
  logItemMasterSync_(config, 'Received full-sync finalize response', {
    ...
    targetResults: finalizeResult.results,
  });
...
function postSyncPayloadToTargets_(config, payload, contextLabel) {
  ...
  return {
    primary: successes[successes.length - 1].parsed,
    results: results,
    failures: failures,
  };
}
...
function performFlush_(config, idHeader) {
  ...
  try {
    ...
    return {
      ...parsedResponse,
      partialFailure: targetResult.failures.length > 0,
      targetResults: targetResult.results,
    };
  } catch (error) {
