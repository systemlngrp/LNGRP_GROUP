...
async function generateLockedMrrNo(db: mysql.Pool, dateStr?: string) {
...
async function automateInterFirmProduction(db: mysql.Pool, productionId: string, sourceType = "Production") {
  ...
  try {
    ...
    const destinationFirmId = String(production.destinationFirmId || (routes as any[])[0]?.routeDestinationFirmId || orderFirmId).trim();
