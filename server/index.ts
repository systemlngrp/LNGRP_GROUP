...
async function generateProductionFirmNo(
  db: mysql.Pool | mysql.PoolConnection,
  firmId: string | undefined,
  documentCode: "JOB" | "PHP" | "PLATE",
  dateStr?: string,
) {
  const fy = getShortFinancialYear(dateStr);
  if (!fy) throw new Error("Could not generate production job number because date is invalid.");
  ...
  const [rows] = await db.query("SELECT LAST_INSERT_ID() AS nextValue");
  return `${firmPrefix}/${counterKey}/${String(Number((rows as any[])[0]?.nextValue || 0)).padStart(5, "0")}`;
}
...
async function automateInterFirmProduction(
  ...
  try {
    ...
    const eligibleProcessingRecord =
    ...
    if (sourceType === "Production Processing" && normalizedMachine === "Corrugation Liner") {
      ...
      if (orderFirmId === sourceFirmId) { await skip(); return; }
...
async function backfillInterFirmProductionProcessing(db: mysql.Pool) {
  ...
  for (const row of rows as any[]) {
    ...
    if (machineName !== "Corrugation Liner" && machineName !== "Printing") continue;
    try {
...
async function validateLnkiPrintingComponents(db: mysql.Pool | mysql.PoolConnection, processing: any) {
...
async function getSafeLnkiPrintingValidation(db: mysql.Pool | mysql.PoolConnection, processing: any) {
  try {
    ...
    return { ready: false, warning: "Printing report saved. " + (error as Error).message + " PHP/Plate billing was skipped." };
  }
}

async function syncLnkiPrintingMasterOutputs(db: mysql.Pool | mysql.PoolConnection, processing: any) {
  if (normalizeMachineName(String(processing.machineName || "")) !== "Printing" ||
      String(processing.completionStatus || "").trim().toLowerCase() !== "full") return;
  const productionId = String(processing.productionId || "").trim();
  if (!productionId) return;
  const [rows] = await db.query(
    "SELECT p.*, o.firmId orderFirmIdResolved, o.erpCode orderErp FROM productions p LEFT JOIN orders_schedule os ON os.id = p.scheduleId LEFT JOIN orders o ON o.id = os.orderId WHERE p.id = ? LIMIT 1",
  ...
  const mainErp = String(production.orderErp || production.masterErp || production.erpCode || "").trim();
  if (!mainErp) return;
  for (const component of [{ table: "php_item_master" }, { table: "plate_item_master" }] as const) {
    const [componentRows] = await db.query(
      "SELECT id FROM " + component.table + " WHERE LOWER(TRIM(COALESCE(masterItemNameErpCode, ''))) = LOWER(TRIM(?)) OR LOWER(TRIM(COALESCE(erpItemCode, ''))) = LOWER(TRIM(?)) LIMIT 1",
      [mainErp, mainErp]
    );
    const master = (componentRows as any[])[0];
    if (master?.id) {
      await db.query(
        "UPDATE " + component.table + " SET productionOutputQty = ?, updatedBy = ?, updateTimestamp = ? WHERE id = ?",
        [Number(processing.qty || 0), processing.updatedBy || null, processing.updateTimestamp || new Date().toISOString(), master.id]
      );
    }
  }
}
...
async function backfillProductionJobNumbers(db: mysql.Pool) {
  ...
  const maxByCounter = new Map<string, number>();
  let changed = 0;
  for (const row of rows as any[]) {
      ...
      try {
        ...
        if (tableName === "production_processing") {
          ...
          if (data.machineName === "Corrugation Liner" && completionStatus === "Full") {
            ...
            const [corrugationRows] = await db.query(
              "SELECT COALESCE(SUM(qty), 0) AS fullQty FROM production_processing WHERE productionId = ? AND LOWER(TRIM(machineName)) = 'corrugation liner' AND LOWER(TRIM(COALESCE(completionStatus, 'Full'))) = 'full' AND id <> ?",
              [data.productionId, data.id]
            );
            const fullCorrugationQty = Number((corrugationRows as any[])[0]?.fullQty || 0) + Number(data.qty || 0);
            const [actualRows] = await db.query("SELECT actualPaperUsed FROM productions WHERE id = ? LIMIT 1", [data.productionId]);
            const actualPaperUsed = Number((actualRows as any[])[0]?.actualPaperUsed || 0);
