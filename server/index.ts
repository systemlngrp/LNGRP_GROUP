...
async function generateLockedGateEntryNo(db: mysql.Pool | mysql.PoolConnection, dateStr?: string) {
  const fy = getShortFinancialYear(dateStr);
  if (!fy) throw new Error("Could not generate Gate Entry number because date is invalid.");
  const key = `GE/${fy}`;
  await db.query(
    `INSERT INTO transaction_counters (counterKey, lastValue)
  ...
  try {
    ...
    for (const [supplierId, groupLines] of groups.entries()) {
      ...
      const [poRows] = await conn.query(
        "SELECT poNo FROM `purchase_orders` WHERE poNo LIKE ? AND (firmId = ? OR firmId IS NULL OR TRIM(firmId) = '') ORDER BY CAST(SUBSTRING_INDEX(poNo,'/',-1) AS UNSIGNED) DESC LIMIT 1",
        [likePattern, purchaseFirmId]
      );
      let lastNum = 0;
