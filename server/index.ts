...
async function migrateReelInventoryToUnitOne(db: mysql.Pool) {
  ...
  try {
    ...
    `);
    for (const row of legacyOpeningRows as any[]) {
      const materialId = String(row.materialId || "");
      const qty = Number(row.qty || 0);
      const value = Number(row.value || 0);
      const rate = qty > 0 ? Number((value / qty).toFixed(2)) : 0;
...
app.get("/api/npd-firm-wise", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: (error as Error).message || "Failed to fetch firm-wise NPD items" });
  }
});
...
app.use("/api", (req, res, next) => {
  ...
  if (!isMutation || isReadOnlyPost || req.path.startsWith("/auth/") || req.path.startsWith("/realtime/")) return next();
...
async function dropRemovedFirmScopeColumns(db: mysql.Pool, database: string) {
  for (const tableName of REMOVED_FIRM_SCOPE_TABLES) {
    ...
    try {
      ...
      console.warn(`[DB] Could not drop removed firm scope from ${tableName}:`, (err as Error).message);
    }
  }
}
...
function normalizeNpdLookupKey(value: any) {
  return stringOrEmpty(value).toLowerCase().replace(/[^a-z0-9]/g, "");
}

async function resolveNpdId(db: mysql.Pool, ...values: any[]) {
  ...
  const match = (rows as any[]).find((row) =>
    [row.id, row.npdId, row.erp, row.itemName].some((value) => keys.has(normalizeNpdLookupKey(value)))
  );
...
async function fetchFirmWiseNpdItems(db: mysql.Pool, options: { search?: string; limit: number; offset: number; status?: string }) {
  ...
  const addItemKey = (rawKey: any, itemId: string) => {
    ...
    if (key) {
      ...
      if (normalized) keyToItemId.set(normalized, itemId);
    }
  };
  ...
  for (const row of allNpdRows as any[]) {
    const itemId = stringOrEmpty(row.id || row.npdId);
    if (!itemId) continue;
    addItemKey(row.id, itemId);
    addItemKey(row.npdId, itemId);
  ...
  if (itemIds.length) {
    ...
    try {
      ...
      for (const row of processedOutputs as any[]) {
        ...
        if (!isCorrugationLiner && !isPrinting) continue;
  ...
  const rows = base.rows.map((row) => {
    ...
    for (const firm of firms) {
      const source = rowKeys.reduce((found, key) => found || stocks.get(key)?.get(firm.id), null as any);
...
async function ensureInterFirmSchema(db: mysql.Pool, database: string) {
  ...
  for (const table of ["orders", "productions", "production_processing", "gate_entries", "material_in", "material_issues", "invoices", "invoice_line_items"]) {
    for (const [column, type] of INTER_FIRM_COLUMNS) {
      try { await ensureColumnExists(db, database, table, column, type); } catch (error) {
        console.warn(`[DB] Could not ensure ${table}.${column}:`, (error as Error).message);
      }
    }
  }
  for (const table of ["gate_entries", "material_in"]) {
    try { await ensureColumnExists(db, database, table, "firmName", "VARCHAR(255)"); } catch {}
  }
  for (const table of ["php_job_master", "plate_job_master"]) {
    for (const [column, type] of [["firmId", "VARCHAR(36)"], ["firmName", "VARCHAR(255)"], ["orderFirmId", "VARCHAR(36)"], ["sourceFirmId", "VARCHAR(36)"], ["destinationFirmId", "VARCHAR(36)"]] as const) {
      try { await ensureColumnExists(db, database, table, column, type); } catch {}
    }
  }
  for (const table of ["php_loading_slips", "plate_loading_slips"]) {
    ...
    try { await ensureColumnExists(db, database, table, "firmId", "VARCHAR(36)"); } catch {}
  }
  for (const [column, type] of [["routeSourceFirmId", "VARCHAR(36)"], ["routeDestinationFirmId", "VARCHAR(36)"], ["routeSequence", "INT NOT NULL DEFAULT 1"], ["routeActive", "VARCHAR(10) NOT NULL DEFAULT 'No'"]] as const) {
    try { await ensureColumnExists(db, database, "firms", column, type); } catch (error) {
      console.warn(`[DB] Could not ensure firms.${column}:`, (error as Error).message);
    }
  }
  ...
  const unit2 = (firmRows as any[]).find((row) => {
    const name = String(row.firmName || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "");
...
async function ensureInternalFirmSuppliers(db: mysql.Pool, database: string) {
  ...
  const firms = (firmRows as any[]).filter((firm) => {
  ...
  for (const row of autoRows as any[]) {
    const [supplierRows] = await db.query("SELECT id FROM suppliers WHERE firmId = ? LIMIT 1", [String(row.sourceFirmId || "")]);
    const supplierId = String((supplierRows as any[])[0]?.id || "").trim();
    if (supplierId && String(row.supplierId || "") !== supplierId) await db.query("UPDATE gate_entries SET supplierId = ? WHERE id = ?", [supplierId, row.id]);
  }
  ...
  for (const row of autoMrrRows as any[]) {
    const [supplierRows] = await db.query("SELECT id FROM suppliers WHERE firmId = ? LIMIT 1", [String(row.sourceFirmId || "")]);
    const supplierId = String((supplierRows as any[])[0]?.id || "").trim();
    if (supplierId && String(row.supplierId || "") !== supplierId) await db.query("UPDATE material_in SET supplierId = ? WHERE id = ?", [supplierId, row.id]);
  }
}

async function backfillSystemGeneratedMrrApprovals(db: mysql.Pool) {
...
async function generateFirmDocumentNo(
...
async function generateLockedGateEntryNo(db: mysql.Pool | mysql.PoolConnection, dateStr?: string, firmId?: string) {
...
async function ensureInterFirmAutomationSchema(db: mysql.Pool, database: string) {
  ...
  for (const table of ["php_loading_slips", "plate_loading_slips"]) {
    ...
    for (const [column, type] of INTER_FIRM_COLUMNS) await ensureColumnExists(db, database, table, column, type);
  }
}

async function automateInterFirmProduction(
  db: mysql.Pool,
  sourceId: string,
  ...
  if (!providedConnection) await ensureInterFirmAutomationSchema(db, database);

  const conn = providedConnection || await db.getConnection();
  const ownsConnection = !providedConnection;
  const skip = async () => { if (ownsConnection) await conn.rollback(); };
  try {
    ...
    const normalizeFirm = (value: any) => String(value || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "");
    const findFirm = (matcher: (name: string) => boolean) => (firmRows as any[]).find((row) => matcher(normalizeFirm(row.firmName)));
    const unit2 = findFirm((name) => name.endsWith("unit2") || name.endsWith("unitii"));
    const lnki = findFirm((name) =>
      name === "laxminarayankraftindustries" ||
    ...
    if (sourceType === "Production Processing" && normalizedMachine === "Printing") {
      ...
      if (orderFirmId !== String(lnki.id)) { await skip(); return; }
    ...
    if (sourceType === "Production Processing" && normalizedMachine === "Printing" && orderFirmId === String(lnki.id)) {
      ...
      for (const component of [{ source: "PHP", table: "php_item_master" }, { source: "PLATE", table: "plate_item_master" }] as const) {
        const [componentRows] = await conn.query(
          `SELECT * FROM \`${component.table}\` WHERE LOWER(TRIM(COALESCE(masterItemNameErpCode, ''))) = LOWER(TRIM(?)) OR LOWER(TRIM(COALESCE(erpItemCode, ''))) = LOWER(TRIM(?)) LIMIT 1`,
          [mainErp, mainErp]
        );
    ...
    const [orphanPendingRows] = await conn.query(
      `SELECT ip.* FROM inter_firm_pending_invoices ip
       ...
       ORDER BY ip.updateTimestamp DESC LIMIT 2`,
      [sourceTransactionType, productionId, sourceFirmId, destinationFirmId]
    );
    const orphanCandidates = orphanPendingRows as any[];
    ...
    if (legacySourceId && legacySourceId !== sourceId) {
      for (const table of ["php_loading_slips", "plate_loading_slips", "material_issues", "material_in", "gate_entries"]) {
        await conn.query(
          `UPDATE \`${table}\` SET sourceTransactionId = ?, updateTimestamp = ? WHERE sourceTransactionType = ? AND sourceTransactionId = ? AND destinationFirmId = ?`,
          [sourceId, now, sourceTransactionType, legacySourceId, destinationFirmId]
        );
      }
    ...
    for (const component of componentLines) {
      ...
      const slipNo = String(existing?.slipNo || await generateLoadingSlipNo(conn, table as any, String(sourceRecord.date || production.date || now.slice(0, 10))));
    ...
    const [existingIssue] = await conn.query(
      "SELECT id, issueNo FROM `material_issues` WHERE sourceTransactionType = ? AND sourceTransactionId = ? AND destinationFirmId = ? LIMIT 1",
      [sourceTransactionType, sourceTransactionId, destinationFirmId]
    );
    let sourceIssue = (existingIssue as any[])[0];
    if (!sourceIssue) {
      ...
      await conn.query(
        `INSERT INTO material_issues (id, issueNo, date, issueType, firmId, remarks, interFirmFlow, autoGenerated, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, updatedBy, updateTimestamp)
         VALUES (?, ?, ?, 'Without Job', ?, ?, 'Yes', 'Yes', ?, ?, ?, ?, 'System', ?)`,
        [issueId, issueNo, issueDate, sourceFirmId, `Inter-firm transfer to ${destinationFirmId}`, sourceFirmId, destinationFirmId, sourceTransactionType, sourceTransactionId, now]
      );
      await conn.query(
        `INSERT INTO material_issue_lines (id, materialIssueId, materialId, qty, uom, lastPurchaseRate, openingRate, rate, amount, updatedBy, updateTimestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'System', ?)`,
        [issueLineId, issueId, itemId, qty, sourceRecord.uom || production.uom || "", rate, rate, rate, amount, now]
      );
    ...
    const internalSupplierId = String((internalSupplierRows as any[])[0]?.id || "").trim();
    ...
    let gate = (existingGate as any[])[0];
    if (!gate) {
...
async function backfillInterFirmProductionProcessing(db: mysql.Pool) {
  const [rows] = await db.query(`
  ...
  for (const row of rows as any[]) {
    ...
    try {
      ...
      if (linked?.gateEntryId && linked?.mrrId) repaired += 1;
      else skipped += 1;
    } catch (error) {
      if ((error as Error).message.startsWith("Multiple orphaned inter-firm records")) duplicates += 1;
      else failed += 1;
      console.warn("[INTER-FIRM] Backfill failed for processing report", { id: row.id, machineName, error: (error as Error).message });
    }
  }
...
async function excludeLegacyCorrugationInterFirmRows(db: mysql.Pool, tableName: string, rows: any[]) {
  if (!CORRUGATION_OPERATIONAL_EXCLUSION_TABLES.has(tableName) || !Array.isArray(rows) || rows.length === 0) return rows;
  ...
  const [processingRows] = await db.query(
    `SELECT id FROM \`production_processing\`
     WHERE id IN (${sourceIds.map(() => "?").join(",")})
       AND LOWER(TRIM(COALESCE(machineName, ''))) = 'corrugation liner'`,
    sourceIds
  );
  const obsoleteSourceIds = new Set((processingRows as any[]).map((row) => String(row.id || "").trim()));
  if (obsoleteSourceIds.size === 0) return rows;
  return rows.filter((row) => !obsoleteSourceIds.has(String(row?.sourceTransactionId || "").trim()));
}

async function backfillIndentRequisitionNumbers(db: mysql.Pool) {
...
async function validateLnkiPrintingComponents(db: mysql.Pool, processing: any) {
  ...
  const [rows] = await db.query(
    "SELECT p.*, o.firmId orderFirmIdResolved, o.erpCode orderErp FROM productions p LEFT JOIN orders_schedule os ON os.id = p.scheduleId LEFT JOIN orders o ON o.id = os.orderId WHERE p.id = ? LIMIT 1",
    [productionId]
  );
  const production = (rows as any[])[0];
  if (!production) return;
  const [firmRows] = await db.query("SELECT id, firmName FROM firms");
  const lnki = (firmRows as any[]).find((row) => String(row.firmName || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "") === "laxminarayankraftindustries");
...
async function ensureReelMovementOwnershipSchema(db: mysql.Pool, database: string) {
  const columns: Array<[string, string, string]> = [
    ["material_issues", "firmId", "VARCHAR(36)"],
...
function normalizeWorkflowStatus(tableName: string, row: any) {
  ...
  if (tableName === "material_in") {
    const isSystemGenerated =
      String(normalized.autoGenerated || "").trim().toLowerCase() === "yes" ||
  ...
  if (tableName === "productions") {
    ...
    else if (currentStatus === "Pending Consumption") normalized.status = currentStatus;
...
async function ensureUniquePackingSlipReelNoIndex(db: mysql.Pool, database: string) {
  ...
  try {
    ...
    console.warn(`[DB] Could not ensure unique packing-slip reel number index:`, (err as Error).message);
  }
}

async function ensureUniqueMachineNameIndex(db: mysql.Pool, database: string) {
  ...
  try {
    ...
    console.warn("[DB] Could not ensure unique machine-name index:", (err as Error).message);
  }
}
...
async function generateStandaloneProductionJobNumber(
  db: mysql.Pool,
  tableName: "php_job_master" | "plate_job_master",
  prefix: "PHP" | "PLATE",
  dateValue: unknown
) {
  const fy = getShortFinancialYear(String(dateValue || "")) || getShortFinancialYear();
  const [rows] = await db.query(
    `SELECT transactionNo FROM \`${tableName}\` WHERE transactionNo LIKE ? ORDER BY CAST(SUBSTRING_INDEX(transactionNo, '/', -1) AS UNSIGNED) DESC LIMIT 1`,
    [`${prefix}/${fy}/%`]
  );
  const last = Number.parseInt(String((rows as any[])[0]?.transactionNo || "").split("/").pop() || "0", 10) || 0;
...
await db.query(`
      ...
      for (const tableName of ["orders", "orders_schedule"]) {
        try {
          ...
          console.warn(`[DB] Could not ensure firmName on ${tableName}:`, (err as Error).message);
        }
      }
      ...
      try {
        ...
        console.warn(`[DB] Could not backfill indent firm names:`, (err as Error).message);
      }
  ...
  return {
    getAll: async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        const operationalRows = await excludeLegacyCorrugationInterFirmRows(db, tableName, rows as any[]);
    ...
    upsert: async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        if (["php_job_master", "plate_job_master"].includes(tableName)) {
          ...
          const normalizeFirmName = (value: unknown) => String(value || "").trim().toLowerCase().replace(/[^a-z0-9]/g, "");
          const unit2 = (unitRows as any[]).find((row) => {
          ...
          if (!unit2?.id) return res.status(400).json({ error: "Unit-II firm mapping is missing." });
        ...
        if (tableName === "material_issues" || tableName === "material_returns") {
          ...
          if (recordId) {
            const [reelRows] = await db.query(
              `SELECT id FROM \`${lineTable}\` WHERE \`${parentColumn}\` = ? LIMIT 1`,
              [recordId]
            );
        ...
        if (tableName === "material_in_packing_slips") {
          ...
          if ((duplicateReelRows as any[])[0]?.id) {
          ...
          if (String((materialRows as any[])[0]?.type || "").trim() === "Reel") {
            const unitOne = await getUnitOneFirm(db);
            if (!unitOne?.id) return res.status(400).json({ error: "Unit-I firm is not configured. Reel packing slips must belong to Unit-I." });
            data.firmId = String(unitOne.id);
        ...
        if (tableName === "machines") {
          ...
          const duplicateMachine = (machineRows as any[]).find(
        ...
        if (tableName === "production_processing") {
          ...
          if (data.machineName === "Corrugation Liner" && completionStatus === "Full") {
            ...
            const wastageProduction = (wastageProductionRows as any[])[0] || {};
          ...
          const isNewProcessingEntry = !(existingProcessingRows as any[])[0]?.id;
        ...
        if (tableName === "reel_stock_taker_logs") {
          const unitOne = await getUnitOneFirm(db);
          if (!unitOne?.id) return res.status(400).json({ error: "Unit-I firm is not configured. Reel stock taking must belong to Unit-I." });
        ...
        if (tableName === "indents") {
          try {
            if (!data.indentNo) {
              ...
              const [rows] = await db.query(
                "SELECT indentNo FROM `indents` WHERE indentNo LIKE ? OR indentNo LIKE ? ORDER BY CAST(SUBSTRING_INDEX(indentNo,'/',-1) AS UNSIGNED) DESC LIMIT 1",
                [likePattern, rqLikePattern]
              );
              let lastNum = 0;
              if ((rows as any[]).length > 0) {
        ...
        const firmDocumentFields: Record<string, { field: string; code: string; dateField: string }> = {
        ...
        if (firmDocument && String(data.id || "").trim()) {
          ...
          if (!(existingDocumentRows as any[])[0]) {
            ...
            data[firmDocument.field] = await generateFirmDocumentNo(
              db,
              firmId,
              firmDocument.code,
        ...
        if (tableName === "invoice_line_items") {
          try {
            ...
            console.warn("[DB] Could not make invoice_line_items.loadingSlipId optional:", (error as Error).message);
          }

          const loadingSlipId = String(data.loadingSlipId || "").trim();
          const isAutoInterFirmLine =
            String(data.interFirmFlow || "").toLowerCase() === "yes" ||
          ...
          if (loadingSlipId && isAutoInterFirmLine) {
            ...
            if ((loadingSlipRows as any[]).length === 0) {
        ...
        const schemaName = String((currentDatabaseRows as any[])[0]?.db || process.env.DB_NAME || "u380633007_Inpidata");
        ...
        if (tableName === "productions") {
          ...
          if (resolvedNpdId) data.npdId = resolvedNpdId;
        }
        ...
        const processingMachineName = tableName === "production_processing"
          ...
          : "";
        const isAtomicInterFirmProcessing =
          tableName === "production_processing" &&
        ...
        if (tableName === "invoices" && String(data.interFirmFlow || "").trim().toLowerCase() === "yes") {
          ...
          if (pendingId) {
            ...
            const mrrId = String((pendingRows as any[])[0]?.linkedMrrId || data.linkedMrrId || "").trim();
            if (mrrId) {
    ...
    delete: async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        if (["productions", "production_processing"].includes(tableName)) {
          ...
          if (Number((linkedRows as any[])[0]?.cnt || 0) > 0) {
...
app.post("/api/materials/opening-reels/bulk", async (req, res) => {
  ...
  try {
    ...
    const normalizedRows: OpeningBulkRow[] = [];
    const uploadedReelRows = new Map<string, number>();
    const materialSignatureByErp = new Map<string, string>();
    const firstPopulatedValue = (...values: unknown[]) =>
    ...
    const rowsByErp = new Map<string, OpeningBulkRow[]>();
    normalizedRows.forEach((row) => {
    ...
    for (const [erpCode, rowsForMaterial] of rowsByErp.entries()) {
      ...
      for (const row of rowsForMaterial) {
        ...
        if (existingSlip) {
          ...
          await conn.query(
            `UPDATE \`material_in_packing_slips\` SET firmId = ?, supplierId = ?, materialInId = 'OPENING', materialLineId = ?, materialId = ?, ourReelNo = ?, supplierReelNo = ?, weightKg = ?, openingRate = ?, updatedBy = ?, updateTimestamp = ? WHERE id = ?`,
            [row.firmId, row.supplierId, existingSlip.id, materialId, row.ourReelNo, row.supplierReelNo || existingSlip.supplierReelNo || null, row.weightKg, row.openingRate, actor, timestamp, existingSlip.id]
          );
          updatedReels += 1;
        } else {
          const slipId = crypto.randomUUID();
          await conn.query(
            `INSERT INTO \`material_in_packing_slips\` (id, firmId, supplierId, materialInId, materialLineId, materialId, ourReelNo, supplierReelNo, weightKg, openingRate, updatedBy, updateTimestamp) VALUES (?, ?, ?, 'OPENING', ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    for (const materialId of Array.from(affectedMaterialIds).filter(Boolean)) {
      const [openingRows] = await conn.query(
        `SELECT COALESCE(SUM(weightKg), 0) AS openingQty,
    ...
    return res.status(statusCode).json({ error: (error as Error).message || "Opening-stock upload failed." });
  } finally {
    conn.release();
  }
});
app.post("/api/materials/reel-materials/bulk", async (req, res) => {
  ...
  const inputRows = Array.isArray(req.body?.rows) ? req.body.rows : [];
  if (inputRows.length === 0) return res.status(400).json({ error: "No Reel material rows were provided." });
  ...
  try {
    ...
    if (!reelGroup) {
      ...
      await conn.query(
        "INSERT INTO `material_groups` (`id`, `name`, `updatedBy`, `updateTimestamp`) VALUES (?, 'Reel', ?, ?)",
        [reelGroup.id, actor, timestamp]
      );
    }

    type ReelMaterialBulkRow = { rowNumber: number; erpCode: string; size: number; gsm: number; bf: number; color: string };
    const normalizedRows: ReelMaterialBulkRow[] = [];
    const materialSignatureByErp = new Map<string, string>();
    const materialRowByErp = new Map<string, ReelMaterialBulkRow>();
    inputRows.forEach((raw: any, index: number) => {
      const rowNumber = Number(raw?.rowNumber) || index + 2;
      ...
      if (unsupportedField) fail(`Row ${rowNumber}: ${unsupportedField[0]} is not supported in the Reel Material upload.`);
      ...
      const priorSignature = materialSignatureByErp.get(erpCode);
      if (priorSignature && priorSignature !== signature) fail(`Row ${rowNumber}: Repeated ERP Code ${erpCode} must use the same Size, GSM, BF, and Color values.`);
      materialSignatureByErp.set(erpCode, signature);
      const normalizedRow = { rowNumber, erpCode, size, gsm, bf, color };
      normalizedRows.push(normalizedRow);
      if (!materialRowByErp.has(erpCode)) materialRowByErp.set(erpCode, normalizedRow);
    });
    ...
    for (const row of materialRowByErp.values()) {
      ...
      if (existingMaterial) {
        await conn.query(
          `UPDATE \`materials\` SET type = 'Reel', erpCode = ?, name = ?, uom = 'KGS', materialGroupId = ?, color = ?, size = ?, gsm = ?, bf = ?, updatedBy = ?, updateTimestamp = ? WHERE id = ?`,
          [row.erpCode, displayName, reelGroup.id, row.color, row.size, row.gsm, row.bf, actor, timestamp, materialId]
        );
        updatedMaterials += 1;
      } else {
        await conn.query(
          `INSERT INTO \`materials\` (id, type, erpCode, name, uom, materialGroupId, color, size, gsm, bf, active, updatedBy, updateTimestamp) VALUES (?, 'Reel', ?, ?, 'KGS', ?, ?, ?, ?, ?, 'Yes', ?, ?)`,
          [materialId, row.erpCode, displayName, reelGroup.id, row.color, row.size, row.gsm, row.bf, actor, timestamp]
        );
        createdMaterials += 1;
      }
    }
    ...
    return res.status(statusCode).json({ error: (error as Error).message || "Reel material upload failed." });
  } finally {
    conn.release();
  }
});
...
app.post("/api/settings/clear-core-job-data", async (req, res) => {
  ...
  try {
    const [oldBackupRows] = await db.query(
      "SELECT TABLE_NAME AS tableName FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME LIKE 'core_backup_%'"
    );
    for (const row of oldBackupRows as any[]) {
...
app.post("/api/settings/clear-transactional-data", async (req, res) => {
  const user = await getRequestUser(req);
  if (!user) return res.status(401).json({ error: "Unauthorized" });
  if (user.role !== "Admin") return res.status(403).json({ error: "Only administrators can clear transactional data." });
  ...
  try {
    ...
    for (const table of TRANSACTIONAL_RESET_TABLES) {
      ...
      counts[table] = Number((result as any)?.affectedRows || 0);
    }
    await conn.commit();
    ...
    return res.status(500).json({ error: (error as Error).message || "Failed to clear transactional data." });
  } finally {
    conn.release();
  }
});
...
app.get("/api/purchase-orders/pending-indent-lines", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (requestedFirmId) await assertRequestFirm(db, requestedFirmId);
    const [rows] = await db.query(`
      SELECT 
        il.id as indentLineId,
        il.indentId,
...
app.post("/api/purchase-orders/create-from-indent-lines", async (req, res) => {
  ...
  try {
    ...
    if (!purchaseFirmId) purchaseFirmId = String((dbRows as any[])[0]?.firmId || "").trim();
    if (!purchaseFirmId) return res.status(400).json({ error: "Purchase firm is required." });
    ...
    for (const [supplierId, groupLines] of groups.entries()) {
      ...
      await conn.query(
        "INSERT INTO `purchase_orders` (id, firmId, poNo, indentId, supplierId, poDate, requiredDate, totalQty, totalAmount, taxableAmount, cgst, sgst, igst, roundOff, grandTotal, remarks, status, updatedBy, updateTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          purchaseOrderId,
          purchaseFirmId,
          poNo,
...
app.post("/api/get-pending-job-closure", async (req, res) => {
  ...
  try {
    ...
    const [processingRows] = productionIds.length
      ? await db.query(
          `SELECT productionId, machineId, machineName, shift, qty, operatorId, date, completionStatus FROM \`production_processing\` WHERE productionId IN (${productionIds
            .map(() => "?")
            .join(",")})`,
          productionIds
        )
...
app.post("/api/reel-transfers/execute", async (req, res) => {
  const user = await getRequestUser(req);
  if (!user) return res.status(401).json({ error: "Unauthorized" });
  if (!hasPermission(user, "/material-movement/reel-transfer")) return res.status(403).json({ error: "Forbidden" });
  ...
  try {
    ...
    const [productionRows] = await conn.query(
      "SELECT * FROM `productions` WHERE `id` IN (?, ?) FOR UPDATE",
      [sourceProductionId, targetProductionId]
    );
    const productions = productionRows as any[];
    ...
    const [processingRows] = await conn.query(
      "SELECT `productionId`, `machineName`, `qty`, `completionStatus`, `date`, `updateTimestamp` FROM `production_processing` WHERE `productionId` IN (?, ?)",
      [sourceProductionId, targetProductionId]
    );
    const processing = processingRows as any[];
    const corrugationRows = (productionId: string) => processing.filter((row) =>
      String(row.productionId) === productionId &&
    ...
    const windowHours = Number((settingRows as any[])[0]?.reelTransferWindowHours || 12);
    ...
    const [targetIssueRows] = await conn.query(
      "SELECT COUNT(*) AS `count` FROM `material_issue_reel_lines` WHERE `productionId` = ? OR LOWER(TRIM(`jobNo`)) = LOWER(TRIM(?))",
      [targetProductionId, targetJobNo]
    );
    if (Number((targetIssueRows as any[])[0]?.count || 0) > 0) throw new Error("The target job already has a reel issue.");

    transferStage = "locking source reel issues";
    const [issueRows] = await conn.query(
      `SELECT ril.*, COALESCE(mil.rate, mil.lastPurchaseRate, mil.openingRate, 0) AS rate,
       ...
       WHERE ril.\`productionId\` = ? OR LOWER(TRIM(ril.\`jobNo\`)) = LOWER(TRIM(?)) FOR UPDATE`,
      [sourceProductionId, sourceJobNo]
    );
    transferStage = "locking source reel returns";
    const [returnRows] = await conn.query(
      "SELECT * FROM `material_return_reel_lines` WHERE `productionId` = ? OR LOWER(TRIM(`jobNo`)) = LOWER(TRIM(?)) FOR UPDATE",
      [sourceProductionId, sourceJobNo]
    );
    ...
    const issueId = crypto.randomUUID();
    const transferNo = await generateSimpleTransactionNumber(conn as any, "reel_transfers", "transferNo", "RT", transferDate);
    const returnNo = await generateSimpleTransactionNumber(conn as any, "material_returns", "returnNo", "MR", transferDate);
    const issueNo = await generateSimpleTransactionNumber(conn as any, "material_issues", "issueNo", "MIS", transferDate);
    ...
    for (const production of [source, target]) {
      ...
      const [usageRows] = await conn.query(
        `SELECT
          ...
          COALESCE((SELECT SUM(\`weightKg\`) FROM \`material_return_reel_lines\` WHERE \`productionId\` = ? OR LOWER(TRIM(\`jobNo\`)) = LOWER(TRIM(?))), 0) AS \`usage\``,
        [productionId, jobNo, productionId, jobNo]
      );
      const usage = Math.max(0, Number((usageRows as any[])[0]?.usage || 0));
...
async function requirePurchaseOrderReadAccess(req: express.Request, res: express.Response): Promise<AuthUser | null> {
  const user = await getRequestUser(req);
  if (!user) {
    res.status(401).json({ error: "Unauthorized" });
    return null;
...
app.get("/api/indents/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (!indentId) return res.status(400).json({ error: "Indent ID is required" });

    const db = await getPool();
    if (!db) return res.status(500).json({ error: "DB connection not available" });
    const [rows] = await db.query("SELECT * FROM `indents` WHERE `id` = ? LIMIT 1", [indentId]);
    const indent = (rows as any[])[0];
...
app.get("/api/indent-lines", requireAuth, async (req, res) => {
  try {
    ...
    if (!(indentRows as any[])[0]) return res.status(404).json({ error: "Indent not found" });
