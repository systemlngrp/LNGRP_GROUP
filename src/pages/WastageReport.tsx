...
const columns = [
  "Date","Job No","Item / ERP","Corr Qty","Print Qty","Production Qty","Required Reel KG","Plan Qty","Paper KG/Box",
  "Warpage B","Warpage KG","Delamination B","Delamination KG","Misalignment B","Misalignment KG","Sheer Cutter B","Sheer Cutter KG",
  "2-Ply/Paper KG","Deckel KG","No Hisab B","No Hisab KG","Sheet Plant Wastage KG",
  "Slotting","Delam Print","Misalign Print","Dry Sheets","Warp","Misprinting","Job Setting","P Wastage B","P Wastage KG",
  "C Wastage %","Total C Wastage KG","Total Wastage %","Printing Wastage %","Combined Total Wastage %",
];
...
export function WastageReport() {
  ...
  const exportRows = rows.map((r, i) => ({
    "SL No": i + 1, Date: formatDate(r.date), "Job No": r.jobNo, Item: r.itemName, ERP: r.erp, "Corrugation Qty": r.corrugationQty, "Printing Qty": r.printingQty, "Production Qty": r.qty, "Required Reel KG": r.requiredReelKg, "Plan Qty": r.planQuantity, "Paper KG/Box": r.paperKgPerBox,
    "Warpage Boxes": r.warpageBoxes, "Warpage KG": r.warpageKg, "Delamination Boxes": r.delaminationBoxes, "Delamination KG": r.delaminationKg, "Misalignment Boxes": r.misalignmentBoxes, "Misalignment KG": r.misalignmentKg, "Sheer Cutter Boxes": r.sheerCutterBoxes, "Sheer Cutter KG": r.sheerCutterKg, "2-Ply/Paper KG": r.twoPlyPaperKg, "Deckel KG": r.deckelWastageKg, "No Hisab Boxes": r.noHisabBoxes, "No Hisab KG": r.noHisabKg, "Sheet Plant Wastage KG": r.sheetPlantWastageKg,
    Slotting: r.slotting, "Delamination Printing": r.delaminationPrinting, "Misalignment Printing": r.misalignmentPrinting, "Dry Sheets": r.drySheets, Warp: r.warp, Misprinting: r.misprinting, "Job Setting": r.jobSetting, "P Wastage Boxes": r.pWastageBoxes, "C Wastage %": r.cWastagePercent, "Total C Wastage KG": r.totalCWastageKg, "Total Wastage %": r.totalWastagePercent, "Combined Total Wastage %": r.combinedTotalWastagePercent,
  }));
  const exportExcel = () => { const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{ Metric: "Total Jobs", Value: summary.recordCount }, { Metric: "Sheet Plant Wastage KG", Value: summary.sheetPlantWastageKg }, { Metric: "Total C Wastage KG", Value: summary.totalCWastageKg }, { Metric: "Total Wastage %", Value: summary.totalWastagePercent }]), "Summary"); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportRows), "Job Wastage"); XLSX.writeFile(wb, "Wastage_Report.xlsx"); };
  const exportPdf = () => { const doc = new jsPDF("l", "mm", "a4"); doc.text("Job-Wise Production Wastage Report", 10, 10); autoTable(doc, { head: [["Job No", "Item", "No Hisab KG", "Sheet Wastage KG", "P Wastage B", "C Wastage %", "Total C KG", "Total %"]], body: rows.map(r => [r.jobNo, r.itemName, f(r.noHisabKg), f(r.sheetPlantWastageKg), f(r.pWastageBoxes), f(r.cWastagePercent) + "%", f(r.totalCWastageKg), f(r.totalWastagePercent) + "%"]), startY: 16, styles: { fontSize: 7 }, headStyles: { fillColor: [55, 45, 190] } }); doc.save("Wastage_Report.pdf"); };
  ...
  return <div className="space-y-4"><h2 className="border-b border-black pb-3 text-xl font-bold uppercase">Job-Wise Production Wastage Report</h2>
    <div className="grid gap-3 md:grid-cols-4"><Card label="Total Jobs" value={q(summary.recordCount)} /><Card label="Sheet Plant Wastage KG" value={f(summary.sheetPlantWastageKg)} /><Card label="Total C Wastage KG" value={f(summary.totalCWastageKg)} /><Card label="Total Wastage %" value={f(summary.totalWastagePercent) + "%"} /></div>
    ...
    <div className="overflow-auto rounded border-2 border-black bg-white"><table className="min-w-[3500px] w-full border-collapse text-[11px]"><thead><tr className="bg-indigo-700 text-white">{columns.map(h => <th key={h} className="border border-black px-2 py-2 font-black whitespace-nowrap">{h}</th>)}</tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={columns.length} className="p-8 text-center">No wastage records found.</td></tr> : rows.map(r => <tr key={r.productionId}><td className="border border-black px-2 py-2">{formatDate(r.date)}</td><td className="border border-black px-2 py-2 font-bold">{r.jobNo}</td><td className="border border-black px-2 py-2">{r.itemName} / {r.erp || "-"}</td>{[r.corrugationQty,r.printingQty,r.qty,r.requiredReelKg,r.planQuantity,r.paperKgPerBox,r.warpageBoxes,r.warpageKg,r.delaminationBoxes,r.delaminationKg,r.misalignmentBoxes,r.misalignmentKg,r.sheerCutterBoxes,r.sheerCutterKg,r.twoPlyPaperKg,r.deckelWastageKg,r.noHisabBoxes,r.noHisabKg,r.sheetPlantWastageKg,r.slotting,r.delaminationPrinting,r.misalignmentPrinting,r.drySheets,r.warp,r.misprinting,r.jobSetting,r.pWastageBoxes].map((v,i)=><td key={i} className="border border-black px-2 py-2 text-right">{f(v)}</td>)}{[r.cWastagePercent,r.totalCWastageKg,r.totalWastagePercent,r.combinedTotalWastagePercent].map((v,i)=><td key={i} className="border border-black px-2 py-2 text-right">{i === 0 || i > 1 ? `${f(v)}%` : f(v)}</td>)}</tr>)}</tbody></table></div></div>;
}
