...
const columns = [
  ...
  "Total C Wastage KG","Total Wastage %","Printing Wastage %","Combined Total Wastage %",
];
...
export function WastageReport() {
  ...
  const exportRows = rows.map((r, i) => ({
    ...
    Slotting: r.slotting, "Delamination Printing": r.delaminationPrinting, "Misalignment Printing": r.misalignmentPrinting, "Dry Sheets": r.drySheets, Warp: r.warp, Misprinting: r.misprinting, "Job Setting": r.jobSetting, "P Wastage Boxes": r.pWastageBoxes, "P Wastage KG": r.pWastageKg, "Total C Wastage KG": r.totalCWastageKg, "Total Wastage %": r.totalWastagePercent, "Printing Wastage %": r.printingWastagePercent, "Combined Total Wastage %": r.combinedTotalWastagePercent,
  }));
  ...
  const exportPdf = () => { const doc = new jsPDF("l", "mm", "a4"); doc.text("Job-Wise Production Wastage Report", 10, 10); autoTable(doc, { head: [["Job No", "Item", "Actual KG", "No Hisab KG", "Sheet Wastage KG", "P Wastage B", "P Wastage KG", "Total C KG", "Total %"]], body: rows.map(r => [r.jobNo, r.itemName, f(r.actualPaperUsedKg), f(r.noHisabKg), f(r.sheetPlantWastageKg), f(r.pWastageBoxes), f(r.pWastageKg), f(r.totalCWastageKg), f(r.totalWastagePercent) + "%"]), startY: 16, styles: { fontSize: 7 }, headStyles: { fillColor: [55, 45, 190] } }); doc.save("Wastage_Report.pdf"); };
