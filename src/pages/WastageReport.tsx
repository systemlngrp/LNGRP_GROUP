...
export function WastageReport() {
  ...
  const exportPdf = () => { const doc = new jsPDF("l", "mm", "a4"); doc.text("Job-Wise Production Wastage Report", 10, 10); autoTable(doc, { head: [["Job No", "Item", "Actual KG", "No Hisab KG", "Sheet Wastage KG", "P Wastage B", "Helper %", "C Wastage %", "Total C KG", "Total %"]], body: rows.map(r => [r.jobNo, r.itemName, f(r.actualPaperUsedKg), f(r.noHisabKg), f(r.sheetPlantWastageKg), f(r.pWastageBoxes), f(r.printingWastagePercent) + "%", f(r.cWastagePercent) + "%", f(r.totalCWastageKg), f(r.combinedTotalWastagePercent) + "%"]), startY: 16, styles: { fontSize: 7 }, headStyles: { fillColor: [55, 45, 190] } }); doc.save("Wastage_Report.pdf"); };
