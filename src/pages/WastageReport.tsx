...
const columns = [
  ...
  "C Wastage KG","Total C Wastage KG","Total Wastage %","Printing Wastage %","Combined Total Wastage %",
];
...
export function WastageReport() {
  ...
  const exportRows = rows.map((r, i) => ({
    ...
    Slotting: r.slotting, "Delamination Printing": r.delaminationPrinting, "Misalignment Printing": r.misalignmentPrinting, "Dry Sheets": r.drySheets, Warp: r.warp, Misprinting: r.misprinting, "Job Setting": r.jobSetting, "P Wastage Boxes": r.pWastageBoxes, "P Wastage KG": r.pWastageKg, "C Wastage KG": r.cWastageKg, "Total C Wastage KG": r.totalCWastageKg, "Total Wastage %": r.totalWastagePercent, "Printing Wastage %": r.printingWastagePercent, "Combined Total Wastage %": r.combinedTotalWastagePercent,
  }));
  const exportExcel = () => { const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{ Metric: "Total Jobs", Value: summary.recordCount }, { Metric: "Actual Paper KG", Value: summary.actualPaperUsedKg }, { Metric: "Sheet Plant Wastage KG", Value: summary.sheetPlantWastageKg }, { Metric: "P Wastage KG", Value: summary.pWastageKg }, { Metric: "Total C Wastage KG", Value: summary.totalCWastageKg }, { Metric: "Total Wastage %", Value: summary.totalWastagePercent }]), "Summary"); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportRows), "Job Wastage"); XLSX.writeFile(wb, "Wastage_Report.xlsx"); };
  const exportPdf = () => { const doc = new jsPDF("l", "mm", "a4"); doc.text("Job-Wise Production Wastage Report", 10, 10); autoTable(doc, { head: [["Job No", "Item", "Actual KG", "No Hisab KG", "Sheet Wastage KG", "P Wastage B", "P Wastage KG", "C Wastage KG", "Total C KG", "Total %"]], body: rows.map(r => [r.jobNo, r.itemName, f(r.actualPaperUsedKg), f(r.noHisabKg), f(r.sheetPlantWastageKg), f(r.pWastageBoxes), f(r.pWastageKg), f(r.cWastageKg), f(r.totalCWastageKg), f(r.totalWastagePercent) + "%"]), startY: 16, styles: { fontSize: 7 }, headStyles: { fillColor: [55, 45, 190] } }); doc.save("Wastage_Report.pdf"); };
