...
export function PendingInvoicing() {
  ...
  const [interFirmSubmittingId, setInterFirmSubmittingId] = useState<string | null>(null);
  ...
  const submitInterFirmInvoice = async (pending: InterFirmPendingInvoice) => {
    ...
    try {
      ...
      await postEntity("invoices", {
        ...
        interFirmFlow: "Yes", autoGenerated: "Yes", sourceFirmId: pending.sourceFirmId,
        destinationFirmId: pending.destinationFirmId, orderFirmId: pending.orderFirmId,
        sourceTransactionType: pending.sourceTransactionType, sourceTransactionId: pending.id,
      ...
      alert("Inter-firm invoice generated and linked to the destination MRR.");
    } catch (error) {
      console.error("Failed to generate inter-firm invoice:", error);
      alert((error as Error).message || "Failed to generate inter-firm invoice.");
    } finally { setInterFirmSubmittingId(null); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {interFirmPending.filter((row) => row.status === "Pending").length > 0 && (
          <div className="bg-white border-2 border-indigo-700 rounded shadow-sm overflow-hidden">
            <div className="px-4 py-3 bg-indigo-50 border-b-2 border-indigo-700 font-black uppercase">Inter-Firm Manufacturing Pending</div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] border-collapse">
                <thead className="bg-slate-50 border-b border-black"><tr className="divide-x divide-black">
