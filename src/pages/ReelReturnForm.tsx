...
export function ReelReturnForm({ mode = "manual" }: { mode?: "manual" | "qr" }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!unitOneFirm) return setScannerError("Unit-I is not configured in Firm Master. Reel returns cannot be saved.");
  ...
  return <div className="min-w-0 overflow-hidden rounded border border-black bg-white p-3 text-black shadow-sm md:p-6">
    <div className="mb-4 flex flex-col items-start gap-3 border-b border-black pb-3 sm:flex-row sm:items-center md:mb-6"><div><h2 className="break-words text-lg font-bold uppercase tracking-tight md:text-xl">{mode === "qr" ? "QR Reel Return" : "Manual Reel Return"}</h2></div>{returnTo ? <button type="button" onClick={() => navigate(returnTo)} className="inline-flex w-full items-center justify-center gap-1 rounded border border-black bg-white px-3 py-2 text-xs font-bold uppercase hover:bg-slate-100 sm:ml-auto sm:w-auto"><ArrowLeft size={15} /> Pending Returns</button> : null}</div>
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><Field label="Date" required><input type="date" required value={date} onChange={(event) => setDate(event.target.value)} className="w-full rounded border-2 border-black p-2" /></Field><Field label="Job No." required><Select required disabled={lockJob} options={jobOptions} value={productionId} onChange={handleJobChange} placeholder="Select Job..." /></Field><Field label="Remarks"><input value={remarks} onChange={(event) => setRemarks(event.target.value)} className="w-full rounded border-2 border-black p-2" /></Field></div>
