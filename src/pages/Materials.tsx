...
export function Materials() {
  ...
  useEffect(() => {
    ...
    if (!material) return;
    const firmId = material.type === "Reel" ? String(unitOneFirm?.id || "") : String(formData.firmId || "").trim();
    if (!firmId) {
    ...
    setOpeningReelSupplierId(supplierIds.length === 1 ? supplierIds[0] : "");
  }, [editingId, formData.firmId, isFormOpen, materialFirmOpenings, materials, packingSlips, unitOneFirm?.id]);

  const connectedReelsForCurrentMaterial = useMemo<ConnectedReelRow[]>(() => {
    if (formData.type !== "Reel") return [];
    const currentErpCode = normalizeMaterialErp(formData.erpCode);
    const matchingMaterialIds = new Set(
      materials
        .filter((material) =>
          material.type === "Reel" &&
          (
            (editingId && material.id === editingId) ||
            (!!currentErpCode && normalizeMaterialErp(material.erpCode) === currentErpCode)
          )
        )
    ...
    return packingSlips
      ...
      .map((slip) => ({
        ...
        openingRate: slip.openingRate,
        source: isOpeningReelPackingSlip(slip) ? "Opening" : receiptNoById.get(slip.materialInId) || "MRR / Receipt",
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const firmId = normalizedType === "Reel" ? String(unitOneFirm?.id || "") : String(formData.firmId || "").trim();
    if (normalizedType === "Reel" && !firmId) {
      alert("Unit-I is not configured in Firm Master. Reel opening stock cannot be saved.");
      return;
    }
  ...
  function handleOpeningStockBulkUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (event) => {
      try {
        ...
        alert(
          `Opening-stock reels uploaded successfully. ${Number(result.insertedReels || 0)} opening reel(s) added, ${Number(result.updatedReels || 0)} opening reel(s) updated, ` +
  ...
  function handleReelMaterialBulkUpload(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = async (event) => {
      try {
        ...
        const allowedHeaders = new Set(["ERP Code", "Size", "GSM", "BF", "Color"]);
        const uploadedHeaders = headerRow.map((value) => String(value).trim()).filter(Boolean);
        const unsupportedHeaders = uploadedHeaders.filter((header) => !allowedHeaders.has(header));
        const missingHeaders = ["ERP Code", "Size", "GSM", "BF"].filter((header) => !uploadedHeaders.includes(header));
        if (unsupportedHeaders.length > 0 || missingHeaders.length > 0) {
          throw new Error(`Use the Reel Material template only. ${unsupportedHeaders.length ? `Unsupported column(s): ${unsupportedHeaders.join(", ")}. ` : ""}${missingHeaders.length ? `Missing column(s): ${missingHeaders.join(", ")}.` : ""}`.trim());
        }
        const parsedRows = XLSX.utils.sheet_to_json<Record<string, unknown>>(worksheet, { defval: "" })
          .map((row, index) => ({ ...row, rowNumber: index + 2 }))
          .filter((row) => String(row["ERP Code"] || "").trim() !== "");
        if (parsedRows.length === 0) throw new Error("Enter an ERP Code for at least one Reel material row.");

        setIsUploading(true);
        const token = window.localStorage.getItem("authToken") || "";
        const response = await fetch("/api/materials/reel-materials/bulk", {
          method: "POST",
          headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
          body: JSON.stringify({ rows: parsedRows }),
        });
        const result = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(result.error || "Reel material upload failed.");

        window.dispatchEvent(new CustomEvent("sync-data-materials"));
        alert(
          `Reel materials uploaded successfully. ${Number(result.createdMaterials || 0)} material(s) created and ` +
          `${Number(result.updatedMaterials || 0)} material(s) updated.`
        );
      } catch (error) {
      ...
      {isFormOpen ? (
        <div className="bg-white border border-black rounded-xl p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-7">
              ...
              <div className="space-y-2">
                ...
                {formData.type === "Reel" ? (
                  <div className="w-full rounded border-2 border-indigo-400 bg-indigo-50 px-4 py-3 font-bold text-indigo-900">
                    {unitOneFirm?.firmName || "Unit-I firm is not configured"}
                  </div>
                ) : (
                  <select
                ...
                <p className="text-xs font-semibold text-slate-500">{formData.type === "Reel" ? "Reel opening stock is always posted to Unit-I." : "Select a firm to add or edit opening stock for that firm."}</p>
              </div>
              <div className="space-y-2">
            ...
            {(formData.type === "Reel" ? unitOneFirm?.id : formData.firmId) ? (
              <>
            ...
            {formData.type === "Reel" ? (
              <div className="rounded border-2 border-black bg-slate-50 p-4">
                ...
                {connectedReelsForCurrentMaterial.length > 0 ? (
                  <div className="mt-4 rounded border border-slate-400 bg-white p-3">
                    <div className="mb-2 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                      <div>
                        <h4 className="text-xs font-black uppercase text-slate-700">Existing Reels for this ERP</h4>
                        <p className="text-[11px] font-bold text-slate-500">Receipt reels are shown for reference only. Edit opening reels above.</p>
                      </div>
                      <span className="text-[11px] font-black uppercase text-indigo-700">{connectedReelsForCurrentMaterial.length} Reel(s)</span>
                    </div>
                    <div className="max-h-44 overflow-auto">
                      <table className="min-w-full border-collapse text-xs">
                        <thead className="sticky top-0 bg-slate-100">
                          <tr>
                            {["Our Reel No.", "KG", "Rate", "Source"].map((heading) => (
                              <th key={heading} className="border border-slate-400 px-2 py-1.5 text-left font-black uppercase">{heading}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {connectedReelsForCurrentMaterial.map((row) => (
                            <tr key={row.id}>
                              <td className="border border-slate-300 px-2 py-1.5 font-black">{row.ourReelNo}</td>
                              <td className="border border-slate-300 px-2 py-1.5 text-right font-bold">{round2(row.weightKg).toFixed(2)}</td>
                              <td className="border border-slate-300 px-2 py-1.5 text-right font-bold">{formatOptionalNumber(row.openingRate) || "-"}</td>
                              <td className="border border-slate-300 px-2 py-1.5 font-bold">
                                {row.source}
                                {row.isOpening ? <span className="ml-2 rounded bg-indigo-50 px-1.5 py-0.5 text-[10px] font-black uppercase text-indigo-700">Editable above</span> : null}
                              </td>
                            </tr>
                          ))}
                        </tbody>
              ...
              <div className="rounded border border-indigo-300 bg-indigo-50 px-4 py-3 text-sm font-semibold text-indigo-900">
                {formData.type === "Reel"
                  ? "Opening reel stock is maintained by Unit-I and cannot be assigned to another firm."
