...
import { Company, Firm, GateEntry, GateEntryPhoto, Production, ProductionProcessing, Supplier } from "../types";
...
function formatGateEntryDate(value?: string | null) {
  if (!value) return "";
...
export function GateEntryMaster({ cancelledOnly = false }: GateEntryMasterProps = {}) {
  ...
  const supplierOptions = useMemo(
    () => makeOptions(baseEntries.map((entry) => getSupplierNameById(entry.supplierId, suppliers, companies, firms))),
    [baseEntries, suppliers, companies, firms]
  );
  ...
  const productionById = useMemo(() => new Map(productions.map((row) => [String(row.id), row])), [productions]);
  const processingById = useMemo(() => new Map(processing.map((row) => [String(row.id), row])), [processing]);

  const getJobNo = (entry: GateEntry) => {
    if (String(entry.autoGenerated || "").trim().toLowerCase() !== "yes") return "";
    const sourceId = String(entry.sourceTransactionId || "").trim();
    const processingRow = processingById.get(sourceId);
    const productionId = String(processingRow?.productionId || (String(entry.sourceTransactionType || "").trim().toLowerCase() === "production" ? sourceId : "")).trim();
    const production = productionById.get(productionId);
    return String(production?.transactionNo || production?.jobCardNo || processingRow?.jobNo || "").trim();
  };
  ...
  const filteredEntries = useMemo(
    () =>
      [...baseEntries]
        .filter((entry) => {
          const supplierName = getSupplierNameById(entry.supplierId, suppliers, companies, firms);
          const firmName = getFirmNameById(entry.firmId || entry.destinationFirmId, firms);
          const entryDate = toDateOnly(entry.date);
          if (firmFilter && String(entry.firmId || entry.destinationFirmId || "") !== firmFilter) return false;
          ...
          const haystack = [
            ...
            firmName,
            getJobNo(entry),
            entry.autoGenerated === "Yes" ? "system generated auto" : "manual",
          ]
            .filter(Boolean)
            .join(" ")
            .toLowerCase();
    ...
    [baseEntries, cancelledOnly, searchTerm, suppliers, companies, firms, firmFilter, supplierFilter, truckFilter, dateFrom, dateTo, mrrStatusFilter, processingById, productionById]
  );
  ...
  const getSupplierName = (supplierId: string) => getSupplierNameById(supplierId, suppliers, companies, firms);

  const getPhotoCount = (gateEntryId: string) => gateEntryPhotos.filter((photo) => photo.gateEntryId === gateEntryId).length;

  const hasActiveFilters = Boolean(searchTerm || firmFilter || supplierFilter || truckFilter || dateFrom || dateTo || mrrStatusFilter);
  const clearFilters = () => {
    setSearchTerm("");
    setSupplierFilter("");
    setTruckFilter("");
  ...
  const tableHeadings = cancelledOnly
    ? ["Gate Entry No", "Firm", "Job No", "Date", "Supplier", "Invoice No.", "Invoice Value", "Truck No", "Cancel Remarks", "Photos", "Action"]
    : ["Gate Entry No", "Firm", "Job No", "Date", "Supplier", "Invoice No.", "Invoice Value", "Truck No", "MRR No", "Cancel Remarks", "Photos", "Action"];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`grid gap-3 md:grid-cols-3 ${cancelledOnly ? "xl:grid-cols-[minmax(220px,1.4fr)_repeat(5,minmax(120px,1fr))_auto]" : "xl:grid-cols-[minmax(220px,1.4fr)_repeat(6,minmax(120px,1fr))_auto]"} xl:items-center`}>
        ...
        <button
          ...
          className="h-[42px] whitespace-nowrap rounded border border-black bg-white px-3 py-2 text-sm font-bold text-black hover:bg-slate-50 disabled:cursor-not-allowed disabled:border-slate-300 disabled:bg-slate-100 disabled:text-slate-400"
        >
          Clear Filters
        </button>
      </div>
                ...
                paginatedItems.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id} className={cancelled ? "bg-red-50/50" : "hover:bg-slate-50"}>
                      ...
                      <td className="border border-black px-4 py-3 text-sm font-semibold text-black">{getFirmNameById(entry.firmId || entry.destinationFirmId, firms) || "Unassigned"}</td>
                      <td className="border border-black px-4 py-3 text-sm font-semibold text-black">{getJobNo(entry)}</td>
                      <td className="border border-black px-4 py-3 text-sm text-black">{formatGateEntryDate(entry.date)}</td>
                      ...
                      {!cancelledOnly ? <td className="border border-black px-4 py-3 text-sm text-black">{entry.mrrNo || "-"}</td> : null}
                      <td className="border border-black px-4 py-3 text-xs text-black">
                        {cancelled ? (
                          <div className="max-w-[260px] space-y-1">
                            ...
                            <div>{formatGateEntryDate(entry.cancelledAt)}</div>
                            <div>{entry.cancelledBy || "System User"}</div>
                          </div>
                        ) : ""}
                      </td>
                      <td className="border border-black px-4 py-3 text-sm text-black">{getPhotoCount(entry.id) > 0 ? `${getPhotoCount(entry.id)} Photos` : ""}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[85vh] w-full max-w-5xl overflow-y-auto rounded-[24px] border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <InfoCard label="Date" value={formatGateEntryDate(selectedEntry.date)} />
              <InfoCard label="Firm" value={getFirmNameById(selectedEntry.firmId || selectedEntry.destinationFirmId, firms) || "Unassigned"} />
              <InfoCard label="Job No" value={getJobNo(selectedEntry)} />
              ...
              <InfoCard label="MRR Date" value={selectedEntry.mrrDate ? formatGateEntryDate(selectedEntry.mrrDate) : "Pending"} />
              <InfoCard label="Cancel Remarks" value={selectedEntry.cancelReason || ""} />
              <InfoCard label="Cancelled By" value={selectedEntry.cancelledBy || "-"} />
              <InfoCard label="Cancelled At" value={formatGateEntryDate(selectedEntry.cancelledAt)} />
              <InfoCard label="Photos" value={selectedPhotos.length > 0 ? String(selectedPhotos.length) : ""} />
            </div>
...
function getSupplierNameById(supplierId: string, suppliers: Supplier[], companies: Company[], firms: Firm[]) {
