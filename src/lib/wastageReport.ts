...
export type ProductionWastageRow = ProductionProcessing & { source: "Corrugation Liner" | "Printing" | "Combined"; itemName: string; erp: string; actualPaperUsedKg: number; fullCorrugationQty: number; corrugationQty: number; printingQty: number; requiredReelKg: number; planQuantity: number; paperKgPerBox: number; noHisabKg: number; sheetPlantWastageKg: number; cWastageKg: number; cWastagePercent: number; pWastageBoxes: number; pWastageKg: number; totalCWastageKg: number; totalWastagePercent: number; printingWastagePercent: number; combinedTotalWastagePercent: number; totalCorrugationKg: number; totalCorrugationBoxes: number; totalPrintingBoxes: number };
...
export function buildProductionWastageRows(processing: ProductionProcessing[], productions: Production[], filters: ProductionWastageFilters = {}): ProductionWastageRow[] {
  ...
  return Array.from(grouped.entries()).map(([productionId, group]) => {
    ...
    const corrugationBoxes = corrRows.reduce((s, r) => s + num(r.warpageBoxes) + num(r.delaminationBoxes) + num(r.misalignmentBoxes) + num(r.sheerCutterBoxes), 0);
